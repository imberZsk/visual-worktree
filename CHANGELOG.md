...
- GitLab Merge Request 目标分支由单值改为可配置多个，悬停菜单可分别选择 `test`、`master`、`gamma` 等目标分支；升级时自动保留 `1.11.2` 已保存的单目标分支。
...
- 修复 Worktree 页面点击刷新后，任务列表未变化时 AI 价格不会重新计算的问题；手动刷新现在立即绕过短时缓存启动用量统计，不再等待耗时的 Git 状态扫描完成。
- 价格重新计算期间，任务费用和顶部总计明确显示加载状态，避免继续展示旧金额；Command+R 仍可复用短时结果快速恢复价格。
- 修复 macOS 窗口被其他应用遮挡或路径组合下拉状态残留时，整个 Header 进入不可拖动状态的问题；窗口最上沿保留独立拖动区域，失焦后首次按住即可移动窗口。

## [1.11.0] - 2026-08-19

### 新增

- AI 用量计价支持按任务持久化 API Key 专属价格，同一模型通过不同中转 Key 调用时可分别匹配账单；Claude Code 与 Codex 的 Input、Output、Cache write、Cache read 和费用在明细中独立展示。

### 优化

- AI 用量统计移到独立 Worker，首屏延迟执行并合并相同并发请求、短时复用刷新结果；Worktree 页面一次批量加载所有任务，避免每个费用标签重复扫描本地会话造成首次进入、刷新和展开卡顿。
- 任务费用标签和顶部总计仅显示合计价格，详细 Token 与分工具费用保留在悬浮明细；直接人民币模式统一保留 6 位小数，与中转账单精度一致。
- AI 模型设置新增不含完整 API Key 的本地摘要，普通设置读取不再触发 macOS 钥匙串授权；移除 Worktree 环境健康检查及其持久化入口，精简启动与任务操作链路。
- macOS 开发版和安装版统一使用透明 Dock PNG，并允许非活动窗口首次按下直接响应自定义标题栏操作。

### 修复

- 修复 Claude Sonnet 5 内置价格、四类 Token 计价和任务级价格覆盖未统一应用的问题；Codex 的任务级规则与 Claude 通过同一批量汇总链计算。
- 修复看板纵向滚动时状态列头随内容移动的问题；列头固定在看板顶部，横向滚动仍由看板统一承载。
- 修复 Worktree 任务行及费用计算中子节点悬停仍显示箭头的问题，整行可点击区域统一显示手型光标。
- 修复 AI 助手抽屉关闭按钮被 Ant Design 运行时内边距压缩、图标显示不完整的问题。

## [1.10.0] - 2026-08-12

### 新增

- 新增可持久化的任务分类标签，首次默认提供品红色“需求”和红色“BUG”；可在设置中增删、改名、改色、排序和控制显示，并可在 Worktree 标题及看板任务卡片中直接切换。
- AI 智能助手升级为多会话聊天，支持横向 Tab、请求级思考强度、图片与文件附件、工具调用状态、可折叠过程信息及响应耗时展示。

### 优化

- AI 聊天输入区改为紧凑圆角布局，移除聚焦蓝色外框，将发送按钮放到输入区右侧，并以加号按钮统一承载图片和文件选择。
- 冷启动占位图标由 112px 缩小为 80px，并移除图标下方的四点 loading；明暗主题下仍保持窗口几何居中。

### 修复

- 删除 Worktree 前实时检查主仓库和子模块状态；整体干净但包含已初始化子模块时自动绕过 Git 结构限制直接删除，只有真实未提交改动才提示强制删除，其他失败显示实际错误。

## [1.9.0] - 2026-08-09

### 新增

- 看板改为按设置中的动态任务状态逐列展示，列顺序与任务状态顺序一致；设置页可选择显示列，偏好按路径组合持久化。

### 优化

- 动态看板列设置 280px 最小宽度，状态较多时提供横向滚动浏览；滚动层撑满看板剩余高度，任务较少时也能在下方空白区域左右滑动。
- 任务状态和看板列显示配置统一收进展示偏好的“任务状态”卡片设置弹层，看板 Tab 不再额外占用一行设置工具栏；移除固定列和旧看板归类功能，并为每张任务卡片增加与 Worktree Tab 共用持久化状态的置顶操作及任务目录 VSCode 入口；全局滚动区域隐藏原生滚动条外观并保留滚动能力。

### 修复

- 修复设置抽屉覆盖应用 Header 时，右上角关闭按钮仍落入 Electron 窗口拖动区域而无法点击的问题。
- 修复历史任务弹层分页器被列表滚动区裁切，以及翻页后内容高度参与分页测量导致界面持续抖动的问题。
- 修复历史任务第三页删除记录后，因下一条记录高度变化重新计算每页数量而跳到第二页的问题。
- 移除“创建 Worktree”和“设置”文字按钮的重复悬停提示，避免按钮含义已明确时再次遮挡下方内容。
- 领先与落后状态仅比较当前分支对应的远程同名分支，无匹配上游时不再把主分支差异误显示为落后。
- 修复 Claude 任务汇总费用未使用自定义人民币汇率的问题；Token 费用新增持久化的“直接人民币显示”，支持按 1:1 计费并统一以人民币展示。
- Token 统计工具支持同时选择 Claude Code 与 Codex，为每个工具独立配置 Token 单价，并在任务徽标和顶部总计中展示各工具费用及合计。
