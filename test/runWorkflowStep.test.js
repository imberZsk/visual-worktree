...
describe('runWorkflowStep 流式执行工作流步骤命令', () => {
  ...
  it('退出码为 0 但输出高置信错误标记时仍回传失败，避免 UI 成功提示与脚本错误冲突', async () => {
    ...
    fake.stderr.emit(
      'data',
      Buffer.from(
        '[错误] 未传入任务目录参数。用法：bash check-unit-test.sh <任务目录>\n'
      )
    )
    fake.emit('close', 0)
    const res = await p
  ...
  it('退出码为 0 但输出未通过标记时仍回传失败，避免检查脚本漏设退出码', async () => {
    ...
    fake.stdout.emit(
      'data',
      Buffer.from('[未通过] 单测检查未通过：logistics 需要补充单测\n')
    )
    fake.emit('close', 0)
    const res = await p
    expect(res.success).toBe(false)
  ...
  it('退出码为 0 但输出失败标记时仍回传失败，避免 Jira 脚本失败后被自动勾选', async () => {
    ...
    const p = runWorkflowStep({
      command: 'bash comment-branch-jira.sh',
      cwd: '/wt/T',
    })
