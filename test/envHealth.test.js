...
import net from 'net';
import { checkEnvHealth, detectProjectKind, probeTcp } from '../src/core/envHealthService.js';
...
describe('envHealthService', () => {
  ...
  describe('detectProjectKind', () => {
    ...
    it('identifies PHP backend projects from composer and ThinkPHP files', () => {
      ...
      writeFileSync(join(proj, 'composer.json'), JSON.stringify({ require: { php: '>=7.4' } }));
    ...
    it('identifies frontend pc web as frontend instead of fullstack', () => {
      ...
      writeFileSync(join(proj, 'package.json'), JSON.stringify({
        ...
        devDependencies: { webpack: '^5.72.1', 'webpack-dev-server': '^4.9.0' },
      }));
  ...
  describe('checkEnvHealth - project summary', () => {
    it('ignores the default root docs work document directory during project checks', async () => {
      ...
      writeFileSync(join(docsDir, 'package.json'), JSON.stringify({ name: 'docs', scripts: {} }));
    ...
    it('ignores custom root work document directories from config templates', async () => {
      ...
      writeFileSync(join(notesDir, 'package.json'), JSON.stringify({ name: 'notes', scripts: {} }));
      ...
      const result = await checkEnvHealth(taskDir, [], { workDocumentTemplates: templates });
    ...
    it('ignores unknown non frontend/backend project directories during project checks', async () => {
      ...
      writeFileSync(join(superpowersDir, 'package.json'), JSON.stringify({ name: 'superpowers', scripts: { test: 'node tests/run.js' } }));
  ...
  describe('checkEnvHealth - ports', () => {
    it('detects an occupied port declared in scripts', async () => {
      ...
      mkdirSync(join(proj, 'node_modules'), { recursive: true });
      writeFileSync(join(proj, 'package.json'), JSON.stringify({ name: 'c', scripts: { dev: `vite --port ${port}` } }));
      writeFileSync(join(proj, 'package-lock.json'), '{}');
  ...
  describe('checkEnvHealth - edge cases', () => {
    ...
    it('always returns all four check keys', async () => {
      ...
      const result = await checkEnvHealth(taskDir);
      expect(Object.keys(result).sort()).toEqual(['deps', 'git', 'ports', 'projects', 'services', 'summary']);
