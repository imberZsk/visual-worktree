...
import {
  ...
  setVisibilityKey,
  sortPinnedItems,
} from '../src/ui/visibilityLogic.ts'
...
describe('visibilityLogic hidden/pinned prefs', () => {
  ...
  it('toggles hidden and pinned keys without mutating the input', () => {
    ...
    const unhiddenPrefs = setVisibilityKey(
      hiddenPrefs,
      'hidden',
      'TASK-A',
      false
    ...
    const pinnedPrefs = setVisibilityKey(
      unhiddenPrefs,
      'pinned',
      'TASK-C',
      true
  ...
  it('filters hidden items unless showHidden is enabled', () => {
    ...
    expect(
      filterVisibleItems(tasks, prefs, (task) => task.task, false).map(
        (task) => task.task
      )
    ).toEqual(['TASK-A', 'TASK-C'])
    expect(
      filterVisibleItems(tasks, prefs, (task) => task.task, true).map(
        (task) => task.task
      )
...
describe('task title badge visibility', () => {
  it('defaults every task title badge to visible', () => {
    expect(normalizeTaskTitleBadges(undefined)).toEqual(
      DEFAULT_TASK_TITLE_BADGES
    )
  })
