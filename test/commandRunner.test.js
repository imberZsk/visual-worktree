...
describe('buildStepCommand（占位符渲染 + shell 引号包裹）', () => {
  ...
  it('auto 模式下脚本命令缺少占位符时自动追加任务目录参数', () => {
    ...
    const cmd = buildStepCommand(
      'bash check-unit-test.sh',
      { path: '/wt/TASK-A' },
      { taskArgMode: 'auto' }
    )
  ...
  it('appendPath 模式下强制追加任务目录，none 模式下不追加', () => {
    ...
    const forced = buildStepCommand(
      'npm test',
      { path: '/wt/TASK-A' },
      { taskArgMode: 'appendPath' }
    )
    // skipped 存储用户显式选择「不追加」后的命令。
    const skipped = buildStepCommand(
      'bash check-unit-test.sh',
      { path: '/wt/TASK-A' },
      { taskArgMode: 'none' }
    )
...
describe('resolveShell（按平台解析执行 shell）', () => {
  ...
  it('Windows 探测用户级安装的 Git Bash（AppData 路径）', () => {
    ...
    const shell = resolveShell('win32', (p) =>
      p.endsWith('AppData/Local/Programs/Git/bin/bash.exe')
    )
    expect(shell.shell).toBe('bash')
    expect(shell.bashFound).toBe(true)
  ...
  it('省略 existsSyncFn 参数时默认探测不到 Git Bash 并兜底 cmd', () => {
    // 不传第二参，触发默认 existsSyncFn=() => false；win32 下必然兜底 cmd
