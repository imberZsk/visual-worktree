...
describe('scanWorktreesByTask', () => {
  ...
  it('groups worktrees by task directory across projects', async () => {
    ...
    git(
      projA,
      `worktree add -q -b feat/task1-a ${join(worktreesRoot, 'TASK-1', 'projA')}`
    )
    git(
      projB,
      `worktree add -q -b feat/task1-b ${join(worktreesRoot, 'TASK-1', 'projB')}`
    )
    // 任务 TASK-2 只涉及 projA
    git(
      projA,
      `worktree add -q -b feat/task2-a ${join(worktreesRoot, 'TASK-2', 'projA')}`
    )
    ...
    expect(t1.worktrees.map((w) => w.project).sort()).toEqual([
      'projA',
      'projB',
    ])
  ...
  it('keeps the full task name when the task contains slash-separated branch style segments', async () => {
    ...
    git(
      projA,
      `worktree add -q -b feat/slash-task ${join(worktreesRoot, taskName, 'projA')}`
    )
  ...
  it('ignores the main worktree (not under worktreesRoot)', async () => {
    ...
    git(
      projA,
      `worktree add -q -b feat/x ${join(worktreesRoot, 'TASK-X', 'projA')}`
    )
    const tasks = await scanWorktreesByTask(projectsRoot, worktreesRoot)
  ...
  it('源项目根目录不匹配时，直接从任务目录恢复 Git worktree', async () => {
    // unregisteredProjectsRoot 存储不包含真实源仓库的错误配置项目根目录。
    const unregisteredProjectsRoot = join(ctx.root, 'wrong-projects')
    mkdirSync(unregisteredProjectsRoot, { recursive: true })
    // sourceProject 存储真实源仓库；它故意不位于错误配置的项目根目录中。
    const sourceProject = initRepo(join(ctx.root, 'source-project'), 'master')
    // taskWorktreePath 存储任务目录中的真实 Git worktree。
    const taskWorktreePath = join(
      worktreesRoot,
      'TASK-FALLBACK',
      'source-project'
    )
    git(sourceProject, `worktree add -q -b feat/fallback ${taskWorktreePath}`)

    const tasks = await scanWorktreesByTask(
      unregisteredProjectsRoot,
      worktreesRoot
    )
  ...
  it('includes branch and status info per worktree', async () => {
    ...
    git(
      projA,
      `worktree add -q -b feat/s ${join(worktreesRoot, 'TASK-S', 'projA')}`
    )
    const tasks = await scanWorktreesByTask(projectsRoot, worktreesRoot, {
      status: true,
    })
  ...
  it('为任务中的每个项目 worktree 带上 GitLab 网页地址', async () => {
    ...
    git(
      projA,
      `worktree add -q -b feat/gitlab ${join(worktreesRoot, 'TASK-GITLAB', 'projA')}`
    )
