...
describe('normalizeWorkflowSteps', () => {
  ...
  it('保留传入的 key（改名沿用旧 key 以保住勾选态）', () => {
    const out = normalizeWorkflowSteps([{ key: 'start', label: '已改名的开始' }]);
  ...
  it('保留运行策略字段并为旧配置补默认值', () => {
    ...
    const out = normalizeWorkflowSteps([
      ...
      { key: 'b', label: 'B', command: 'npm run build', autoCheckOnSuccess: false, stopOnFailure: false },
    ]);
  ...
  it('保留任务目录参数模式，并为旧配置补 auto 默认值', () => {
    ...
    expect(out.map((step) => step.taskArgMode)).toEqual(['auto', 'none', 'appendPath', 'auto']);
  });
});
...
describe('setStepDoneInMap', () => {
  ...
  it('取消其中一个步骤保留其余', () => {
    const next = setStepDoneInMap({ T1: ['start', 'unit-test'] }, 'T1', 'start', false);
  ...
  it('任务名或步骤 key 缺失时原样返回拷贝', () => {
    expect(setStepDoneInMap({ T1: ['start'] }, '', 'start', true)).toEqual({ T1: ['start'] });
    expect(setStepDoneInMap({ T1: ['start'] }, 'T1', '', true)).toEqual({ T1: ['start'] });
  });

  it('容忍 null/undefined 映射', () => {
    expect(setStepDoneInMap(undefined, 'T1', 'start', true)).toEqual({ T1: ['start'] });
  });
});
...
describe('computeWorkflowProgress', () => {
  ...
  it('已勾选但步骤已从清单删除的不计入 done', () => {
    ...
    const p = computeWorkflowProgress(steps, { T1: ['a', 'deleted-step'] }, 'T1');
    expect(p).toEqual({ done: 1, total: 5 });
  });

  it('空步骤清单返回 0/0', () => {
    expect(computeWorkflowProgress([], { T1: ['a'] }, 'T1')).toEqual({ done: 0, total: 0 });
  });
});
...
describe('loadTaskWorkflowMap / saveTaskWorkflowMap', () => {
  ...
  it('往返一个工作流映射', () => {
    ...
    expect(localStorage.getItem(TASK_WORKFLOW_STORAGE_KEY)).toBe('{"T1":["start","unit-test"]}');
