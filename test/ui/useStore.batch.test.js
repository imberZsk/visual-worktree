...
function resetStore() {
  ...
  mockApi.loadTaskVisibility.mockReset().mockResolvedValue({ hidden: [], pinned: [] });
  mockApi.saveTaskVisibility.mockReset().mockResolvedValue(true);
  mockApi.loadProjectVisibility.mockReset().mockResolvedValue({ hidden: [], pinned: [] });
...
describe('useStore runBatch selection behavior', () => {
  ...
  it('批量拉取结束后清空项目勾选', async () => {
    ...
    const results = [{ success: true }, { success: false, error: 'remote failed' }];
  ...
  it('非拉取批量操作结束后保留项目勾选', async () => {
    ...
    const selectedPaths = ['/repo/a', '/repo/b'];
    useStore.getState().setSelectedPaths(selectedPaths);
    mockApi.batchOperate.mockResolvedValue([{ success: true }, { success: true }]);
...
describe('useStore task link behavior', () => {
  ...
  it('loadTaskLinks 会把旧版单字符串链接规范化为命名链接条目数组', async () => {
    mockApi.loadTaskLinks.mockResolvedValue({ 'TASK-1': 'https://jira.example.com/TASK-1' });
  ...
  it('setTaskLink 保存多条链接并过滤空白重复项', () => {
    useStore.getState().setTaskLink('TASK-1', [
...
describe('useStore visibility behavior', () => {
  ...
  it('隐藏项目时会清掉该项目的现有勾选并持久化', () => {
    ...
    expect(useStore.getState().projectVisibility).toEqual({ hidden: ['/repo/b'], pinned: [] });
    expect(mockApi.saveProjectVisibility).toHaveBeenCalledWith({ hidden: ['/repo/b'], pinned: [] });
  });

  it('置顶任务时会写入任务可见性偏好', () => {
    useStore.getState().setTaskPinned('TASK-A', true);

    expect(useStore.getState().taskVisibility).toEqual({ hidden: [], pinned: ['TASK-A'] });
    expect(mockApi.saveTaskVisibility).toHaveBeenCalledWith({ hidden: [], pinned: ['TASK-A'] });
  });
});
