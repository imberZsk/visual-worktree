 ...
 */
function mockHistoryListPaginationLayout({
  shellHeight = 224,
  itemHeight = 56,
} = {}) {
...
describe('App worktree 流程执行', () => {
  ...
  it('流程步骤执行成功后自动勾选该步骤', async () => {
    ...
    await waitFor(() => {
      expect(useStore.getState().taskWorkflowMap['TASK-A'] || []).toContain(
        'run'
      )
    })
  })
  ...
  it('流程步骤执行结束后持久化最近一次输出缓存', async () => {
    ...
    await waitFor(() =>
      expect(mockApi.saveTaskWorkflowOutput).toHaveBeenCalledTimes(1)
    )
  ...
  it('流程步骤执行失败后取消已完成勾选并显示未通过', async () => {
    ...
    mockApi.runWorkflowStep.mockResolvedValue({
      success: false,
      code: 1,
      stdout: '',
      stderr: 'unit failed',
    ...
    await waitFor(() => {
      expect(useStore.getState().taskWorkflowMap['TASK-A'] || []).not.toContain(
        'run'
      )
    })
    expect(screen.getByText('未通过')).toBeTruthy()
  ...
  it('执行流程步骤时把任务目录参数模式和任务上下文传给主进程', async () => {
    ...
    await waitFor(() =>
      expect(mockApi.runWorkflowStep).toHaveBeenCalledTimes(1)
    )
    ...
    staleConfig.workflowSteps = [
      {
        key: 'jira',
        label: 'JIRA评论项目名:分支',
        command: '/scripts/comment-jira.sh',
        taskArgMode: 'auto',
      },
    ]
    ...
    latestConfig.workflowSteps = [
      {
        key: 'jira',
        label: 'JIRA评论项目名:分支',
        command: '/scripts/comment-branch-jira.sh',
        taskArgMode: 'appendPath',
      },
    ]
    mockApi.loadConfig
  ...
  it('运行全部遇到失败后停止并保留后续步骤未执行', async () => {
    ...
    mockApi.loadConfig.mockResolvedValue(nextConfig)
    mockApi.runWorkflowStep.mockResolvedValueOnce({
      success: false,
      code: 1,
      stdout: '',
      stderr: 'fail',
  ...
  it('历史任务没有归档 docs 时保留 Finder 和 VSCode 禁用占位', async () => {
    ...
    const finderButton = screen.getByTitle(
      '暂无工作记录归档，无法在 Finder 显示'
    )
    // vscodeButton 存储无归档 docs 时的 VSCode 占位按钮，应禁用但仍保留位置。
    const vscodeButton = screen.getByTitle(
  ...
  it('历史任务弹层按容器高度调整 antd List 每页条数', async () => {
    ...
    const restoreLayoutMock = mockHistoryListPaginationLayout({
      shellHeight: 224,
      itemHeight: 56,
    })
  ...
  it('历史任务第二页删除时按完整历史列表下标删除', async () => {
    ...
    try {
      ...
      await waitFor(() =>
        expect(
          screen.getAllByText('移除历史任务「REMOVE-5」？').length
        ).toBeGreaterThan(0)
      )
  ...
  it('历史任务任务名和链接使用统一单行 Tooltip 文本', async () => {
    ...
    const clippedTexts = document.querySelectorAll(
      '.history-task-list .single-line-tooltip-text'
    )
  ...
  it('删除任务前先归档 docs 并把 docsPath 写入历史记录', async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText('删除任务「TASK-A」').length).toBeGreaterThan(
        0
      )
    )
    ...
    expect(mockApi.appendTaskHistory).toHaveBeenCalledWith(
