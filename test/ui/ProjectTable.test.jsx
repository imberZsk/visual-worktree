...
import ProjectTable from '../../src/ui/components/ProjectTable.jsx';
...
function makeProjects() {
  return [
    ...
    { name: 'beta', path: '/repo/beta', isGitRepo: true, isMainBranch: false, hasUncommittedChanges: true, hasUnpushedCommits: false, canPull: false, ahead: 0, behind: 0 },
  ];
}
...
describe('ProjectTable visibility actions', () => {
  ...
  it('渲染隐藏和置顶项目按钮，点击时回调项目路径与目标状态', () => {
    ...
    render(<ProjectTable {...baseProps({ onProjectHiddenChange, onProjectPinnedChange })} />);
  ...
  it('正在隐藏的项目行带退出动画 class', () => {
    const { container } = render(<ProjectTable {...baseProps({
  ...
  it('项目名和分支使用单行 Tooltip 文本，避免长内容撑高表格行', () => {
    ...
    const longBranch = 'feature/PROJ-1001-关联物料费用金额非必填-with-extra-suffix';
    render(<ProjectTable {...baseProps({
      data: [
        {
          ...makeProjects()[0],
