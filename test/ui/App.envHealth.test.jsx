...
const createdTasks = [
  {
    ...
    path: '/wt/TASK-NEW',
    worktrees: [
      { project: 'projA', projectPath: '/src/projA', path: '/wt/TASK-NEW/projA', branch: 'TASK-NEW', prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
    ],
  },
];
...
describe('App 自动环境检查', () => {
  ...
  it('创建 worktree 成功后自动触发环境检查并在任务行显示 loading', async () => {
    ...
    mockApi.checkEnvHealth.mockReset().mockReturnValue(new Promise((resolve) => { resolveEnvHealth = resolve; }));

    renderApp();

    await waitFor(() => expect(screen.getByText('创建 Worktree')).toBeTruthy());
    fireEvent.click(screen.getByText('创建 Worktree'));
    fireEvent.change(screen.getByPlaceholderText('PROJ-1234-需求简述'), { target: { value: 'TASK-NEW' } });
  ...
  it('创建时填写需求链接会保存到任务链接映射', async () => {
    ...
    fireEvent.change(firstLinkInput, { target: { value: 'https://jira.example.com/TASK-NEW' } });
    ...
    await waitFor(() => expect(mockApi.saveTaskLinks).toHaveBeenCalledWith({
      'TASK-NEW': [
      ...
      projects: [
        {
          ...
          checks: {
            ...
            ports: { status: 'ok', message: '未在 scripts 中发现端口声明', fixes: [], skipped: true },
            services: { status: 'ok', message: '无 .env，跳过服务检查', fixes: [], skipped: true },
            git: { status: 'ok', message: '工作区干净', fixes: [] },
          },
        },
      ],
      deps: { status: 'ok', message: '依赖完整', fixes: [] },
      ports: { status: 'ok', message: '未在 scripts 中发现端口声明', fixes: [] },
  ...
  it('自动执行环境检查完成后会保存任务环境状态缓存', async () => {
    mockApi.scanWorktreesByTask.mockReset().mockResolvedValue(createdTasks);
    mockApi.checkEnvHealth.mockReset().mockResolvedValue({
      summary: { status: 'failed', projectCount: 1, issueCount: 2, failedProjects: ['projA'], message: '发现 2 个环境问题' },
      projects: [],
    ...
    await waitFor(() => {
      ...
      expect(lastSavedMap?.['TASK-NEW']?.result?.summary?.message).toBe('发现 2 个环境问题');
    });
  });
  ...
  it('启动后对旧版本环境检查缓存重新执行检查', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        result: {
          summary: { status: 'failed', projectCount: 1, issueCount: 1, failedProjects: ['logistics'], message: '1 个项目存在 1 个环境问题' },
          projects: [
            {
              name: 'logistics',
              ...
              checks: { deps: { status: 'error', message: '未安装 PHP 依赖（缺少 vendor）', fixes: [] } },
            },
          ],
        },
      },
    ...
    mockApi.checkEnvHealth.mockReset().mockResolvedValue({
      summary: { status: 'ok', projectCount: 1, issueCount: 0, failedProjects: [], message: '1 个项目环境正常' },
      projects: [],
      deps: { status: 'ok', message: '检测到 Docker 运行配置，跳过本机 PHP vendor 检查', fixes: [] },
      ports: { status: 'ok', message: 'ok', fixes: [] },
      services: { status: 'ok', message: 'ok', fixes: [] },
      git: { status: 'ok', message: '工作区干净', fixes: [] },
    });
    ...
    await waitFor(() => expect(mockApi.checkEnvHealth).toHaveBeenCalledWith('/wt/TASK-NEW'));
    await waitFor(() => expect(screen.getByText('环境正常')).toBeTruthy());
  ...
  it('启动后对未检查任务自动后台执行环境检查并保持页面可操作', async () => {
    ...
    expect(screen.getByText('创建 Worktree').closest('button').disabled).toBe(false);
    ...
    await waitFor(() => expect(screen.getAllByText('环境正常').length).toBeGreaterThan(0));
  });
  ...
  it('打开旧环境检查缓存时不展示根级 docs 工作文档目录', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        checkedAt: '2026-07-02T06:53:02.412Z',
        result: {
          summary: { status: 'ok', projectCount: 3, issueCount: 0, failedProjects: [], message: '3 个项目环境正常' },
          projects: [
            { name: 'projA', path: '/wt/TASK-NEW/projA', kind: 'frontend', kindLabel: '前端', status: 'ok', issueCount: 0, reasons: ['前端依赖 vite'], checks: {} },
            { name: 'projB', path: '/wt/TASK-NEW/projB', kind: 'backend', kindLabel: '后端', status: 'ok', issueCount: 0, reasons: ['后端依赖 express'], checks: {} },
            { name: 'docs', path: '/wt/TASK-NEW/docs', kind: 'unknown', kindLabel: '未知', status: 'ok', issueCount: 0, reasons: ['未命中常见前端/后端/小程序特征，仅执行通用检查'], checks: {} },
          ],
        },
      },
    });
  ...
  it('启动后将旧缓存里的 Git warning-only 结果显示为环境正常', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        checkedAt: '2026-07-02T06:53:02.412Z',
        result: {
          summary: { status: 'failed', projectCount: 1, issueCount: 1, failedProjects: ['projA'], message: '1 个项目存在 1 个环境问题' },
          projects: [
            {
              name: 'projA',
              path: '/wt/TASK-NEW/projA',
              ...
              reasons: ['前端依赖 vite'],
              checks: {
                git: { status: 'warning', message: '4 个未提交改动', fixes: [] },
              },
            },
          ],
        },
    ...
    const okTag = [...container.querySelectorAll('.ant-tag')]
      .find((tag) => tag.textContent.includes('环境正常'));
  ...
  it('启动后将旧缓存里的空任务目录结果显示为环境正常', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        checkedAt: '2026-07-02T06:53:02.412Z',
        result: {
          summary: { status: 'failed', projectCount: 0, issueCount: 1, failedProjects: [], message: '任务目录下未找到项目' },
          projects: [],
          deps: { status: 'warning', message: '任务目录下未找到项目', fixes: [] },
          ports: { status: 'warning', message: '任务目录下未找到项目', fixes: [] },
          services: { status: 'warning', message: '任务目录下未找到项目', fixes: [] },
          git: { status: 'warning', message: '任务目录下未找到项目', fixes: [] },
        },
      },
    });
    ...
    expect(screen.queryByText('1 个环境问题')).toBeNull();
    fireEvent.click(screen.getByText('环境正常'));
    await waitFor(() => expect(screen.getByText('任务目录下未找到项目')).toBeTruthy());
  });
  ...
  it('打开旧环境检查缓存时不展示 unknown 非前后端项目', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        checkedAt: '2026-07-02T06:53:02.412Z',
        result: {
          summary: { status: 'failed', projectCount: 2, issueCount: 1, failedProjects: ['superpowers'], message: '1 个项目存在 1 个环境问题' },
          projects: [
            { name: 'projA', path: '/wt/TASK-NEW/projA', kind: 'frontend', kindLabel: '前端', status: 'ok', issueCount: 0, reasons: ['前端依赖 vite'], checks: {} },
            {
              name: 'superpowers',
              ...
              checks: { deps: { status: 'error', message: '未安装依赖', fixes: [] } },
            },
          ],
        },
      },
    ...
    await waitFor(() => expect(screen.getByText('1 个项目环境正常')).toBeTruthy());
    expect(screen.getByText('projA')).toBeTruthy();
  ...
  it('旧环境检查缓存缺少版本时点击状态会重新检查并展示刷新结果', async () => {
    ...
    mockApi.loadTaskEnvHealth.mockReset().mockResolvedValue({
      'TASK-NEW': {
        ...
        checkedAt: '2026-07-02T06:53:02.412Z',
        result: {
          summary: { status: 'warning', projectCount: 2, issueCount: 1, failedProjects: ['projA'], message: '1 个项目存在 1 个环境问题' },
          projects: [
            { name: 'projA', path: '/wt/TASK-NEW/projA', kind: 'frontend', kindLabel: '前端', status: 'warning', issueCount: 1, reasons: [], checks: { git: { status: 'warning', message: '旧 Git 问题', fixes: [] } } },
            { name: 'projB', path: '/wt/TASK-NEW/projB', kind: 'frontend', kindLabel: '前端', status: 'ok', issueCount: 0, reasons: [], checks: {} },
          ],
        },
      },
    });
    mockApi.checkEnvHealth.mockReset().mockResolvedValue({
      summary: { status: 'failed', projectCount: 3, issueCount: 1, failedProjects: ['logistics'], message: '1 个项目存在 1 个环境问题' },
      projects: [
        { name: 'projA', path: '/wt/TASK-NEW/projA', kind: 'frontend', kindLabel: '前端', status: 'ok', issueCount: 0, reasons: [], checks: {} },
        { name: 'projB', path: '/wt/TASK-NEW/projB', kind: 'frontend', kindLabel: '前端', status: 'ok', issueCount: 0, reasons: [], checks: {} },
        {
          name: 'logistics',
          path: '/wt/TASK-NEW/logistics',
      ...
      deps: { status: 'error', message: '未安装 PHP 依赖（缺少 vendor）', fixes: [] },
  ...
  it('环境检查详情项目卡片每页只展示两个项目', async () => {
    ...
    expect(pageTwo.compareDocumentPosition(screen.getByText('projA')) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
