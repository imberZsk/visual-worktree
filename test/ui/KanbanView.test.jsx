...
import KanbanView from '../../src/ui/components/KanbanView.jsx';
...
function makeTasks() {
  return [
    { task: 'TASK-PENDING', path: '/wt/TASK-PENDING', worktrees: [{ project: 'a', path: '/wt/TASK-PENDING/a', branch: 'f1' }] },
    { task: 'TASK-DOING', path: '/wt/TASK-DOING', worktrees: [{ project: 'b', path: '/wt/TASK-DOING/b', branch: 'f2', hasUncommittedChanges: true }] },
    { task: 'TASK-DONE', path: '/wt/TASK-DONE', worktrees: [{ project: 'c', path: '/wt/TASK-DONE/c', branch: 'f3' }] },
  ];
}
...
describe('KanbanView', () => {
  it('renders task names (uses task.task, not task.taskName)', () => {
    render(<KanbanView tasks={makeTasks()} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={statusMap} taskBlockerMap={{}} onBlockerChange={() => {}} onTaskClick={() => {}} />);
  ...
  it('groups tasks by manual status, not workflow progress', () => {
    ...
    const { container } = render(<KanbanView tasks={makeTasks()} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={statusMap} taskBlockerMap={{}} onBlockerChange={() => {}} onTaskClick={() => {}} />);
  ...
  it('all tasks fall into 待启动 when no status set', () => {
    ...
    render(<KanbanView tasks={makeTasks()} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={{}} taskBlockerMap={{}} onBlockerChange={() => {}} onTaskClick={() => {}} />);
    // 进行中/已完成两列应都为空态
  ...
  it('shows existing blocker note and allows editing', () => {
    // saved 记录卡点保存回调收到的参数
    let saved = null;
    render(<KanbanView tasks={makeTasks()} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={statusMap} taskBlockerMap={{ 'TASK-DOING': '等待后端联调' }} onBlockerChange={(name, text) => { saved = { name, text }; }} onTaskClick={() => {}} />);
    // 已有卡点应展示出来
  ...
  it('uses a full-width blocker editor with actions below the textarea', () => {
    ...
    expect(actions.compareDocumentPosition(textarea) & Node.DOCUMENT_POSITION_PRECEDING).toBeTruthy();
  ...
  it('calls onTaskClick with task name when title is clicked', () => {
    ...
    let clicked = null;
    render(<KanbanView tasks={makeTasks()} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={statusMap} taskBlockerMap={{}} onBlockerChange={() => {}} onTaskClick={(name) => { clicked = name; }} />);
    fireEvent.click(screen.getByText('TASK-DOING'));
    expect(clicked).toBe('TASK-DOING');
  });

  it('renders empty gracefully with no tasks', () => {
    render(<KanbanView tasks={[]} workflowSteps={steps} taskWorkflowMap={{}} taskStatusMap={{}} taskBlockerMap={{}} onBlockerChange={() => {}} onTaskClick={() => {}} />);
