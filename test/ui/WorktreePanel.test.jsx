...
function makeTasks() {
  return [
    ...
    },
    {
      ...
      path: '/wt/TASK-B',
      worktrees: [
        { project: 'projB', projectPath: '/src/projB', path: '/wt/TASK-B/projB', branch: 'feat-b', prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
      ],
    },
  ];
}
...
describe('WorktreePanel 受控展开', () => {
  it('activeKeys 为 undefined 时回退为全部收起（无面板处于 active）', () => {
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: undefined })} />);
  ...
  it('activeKeys=[TASK-A] 时只展开 TASK-A 面板', () => {
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'] })} />);
  ...
  it('activeKeys=[] 时全部收起', () => {
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: [] })} />);
  ...
  it('用户手动展开/收起会通过 onActiveKeysChange 写回', () => {
    ...
    render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'], onActiveKeysChange: onChange })} />);
...
describe('WorktreePanel 终端与复制路径按钮', () => {
  it('任务级渲染复制路径与终端图标按钮，点击各自回调并传任务目录路径', () => {
    ...
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'], onCopyPath, onOpenTerminal })} />);
    // 任务级按钮已改为纯图标，定位到 extra 区域（面板头右侧）的复制/终端图标按钮
    const extraArea = container.querySelector('.ant-collapse-item-active .ant-collapse-extra');
  ...
  it('任务级渲染 VSCode 打开图标按钮，点击回调并传任务目录路径', () => {
    ...
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'], onOpenVscode })} />);
  ...
  it('任务级 GitLab 图标紧跟 VSCode 后面，单项目时点击打开该项目 GitLab', () => {
    ...
    const tasks = [
      {
        ...
        path: '/wt/TASK-A',
        worktrees: [
          { project: 'projA', projectPath: '/src/projA', path: '/wt/TASK-A/projA', branch: 'feat-a', gitlabUrl, prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
        ],
      },
    ];
    // container 存储渲染后的 DOM 根节点，用于检查任务头 extra 区域的按钮顺序。
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'], tasks, onOpenUrl })} />);
    ...
    const vscodeButtonIndex = buttons.findIndex((button) => button.querySelector('.anticon-vscode'));
    // gitlabButtonIndex 存储 GitLab 按钮在任务级按钮列表中的位置。
    const gitlabButtonIndex = buttons.findIndex((button) => button.querySelector('.anticon-gitlab'));
  ...
  it('worktree 级（展开后）渲染 Finder/VSCode 图标按钮，点击传 worktree 路径', () => {
    ...
    const { container } = render(<WorktreePanel {...baseProps({ activeKeys: ['TASK-A'], onOpenFinder, onOpenVscode })} />);
    // content 存储展开面板内容区，避免误选面板头部的任务级按钮
    const content = container.querySelector('.ant-collapse-content');
      ...
      tasks: [
        {
          ...
          worktrees: [
            { project: 'hybrid-mobile', projectPath: '/src/hybrid-mobile', path: longPath, branch: longBranch, prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
          ],
        },
      ],
    })} />);
...
describe('WorktreePanel 隐藏置顶与标题徽标展示', () => {
  it('任务头渲染隐藏和置顶图标按钮，点击时回调任务名与目标状态', () => {
    ...
    render(<WorktreePanel {...baseProps({ onTaskHiddenChange, onTaskPinnedChange })} />);
    ...
    const tasks = [
      {
        ...
        worktrees: [
          { project: 'projA', projectPath: '/src/projA', path: '/wt/TASK-A/projA', branch: 'feat-a', prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
          { project: 'projB', projectPath: '/src/projB', path: '/wt/TASK-A/projB', branch: 'feat-a', prunable: false, missing: false, hasUncommittedChanges: false, ahead: 0, behind: 0 },
        ],
      },
    ];
...
describe('WorktreePanel 任务链接', () => {
  it('任务标题按项目数、状态、链接、环境、token 用量顺序展示，并使用空心方形项目数', () => {
    ...
    const taskLinkMap = { 'TASK-A': [{ name: 'Jira', url: 'https://jira.example.com/browse/TASK-A' }] };
    ...
    expect(headerText.indexOf('2项目')).toBeLessThan(headerText.indexOf('开发中'));
    expect(headerText.indexOf('开发中')).toBeLessThan(headerText.indexOf('Jira'));
    expect(headerText.indexOf('Jira')).toBeLessThan(headerText.indexOf('2个环境问题'));
    expect(headerText.indexOf('2个环境问题')).toBeLessThan(headerText.indexOf('1.5K·$0.020'));
    ...
    expect(projectCountTag.getAttribute('style')).toContain('background: transparent');
    expect(projectCountTag.getAttribute('style')).toContain('border-radius: 4px');
  });
  ...
  it('任务绑定多条带名称链接时在任务名旁边逐条展示名称，并可打开指定链接', () => {
    ...
    expect(onOpenUrl).toHaveBeenCalledWith('https://larksuite.example.com/docx/abc');
  });
  ...
  it('管理需求链接时默认一行名称和链接输入，可增加输入框并保存多条命名链接', async () => {
    ...
    const { container } = render(<WorktreePanel {...baseProps({
      taskLinkMap: { 'TASK-A': [{ name: '旧 Jira', url: 'https://jira.example.com/old' }] },
      onTaskLinkChange,
    })} />);

    // manageButton 存储任务头右侧的链接管理按钮，区别于标题里的快捷打开图标。
    const taskHeader = [...container.querySelectorAll('.ant-collapse-header')]
      .find((node) => node.textContent.includes('TASK-A'));
    const manageButton = taskHeader.querySelector('.ant-collapse-extra button .anticon-link').closest('button');
    fireEvent.click(manageButton);

    // firstNameInput 存储默认展示的第一条链接名称输入框；已有名称会回填到这一行。
    const firstNameInput = await screen.findByPlaceholderText(TASK_LINK_NAME_PLACEHOLDER);
    ...
    const firstLinkInput = await screen.findByPlaceholderText(TASK_LINK_PLACEHOLDER);
    expect(firstLinkInput.tagName.toLowerCase()).toBe('input');
    expect(firstLinkInput.value).toBe('https://jira.example.com/old');
    fireEvent.change(firstLinkInput, { target: { value: 'https://jira.example.com/TASK-A' } });
    ...
    const saveButton = [...document.querySelectorAll('.ant-popover button')]
...
describe('WorktreePanel 环境检查状态', () => {
  ...
  it('任务行用黄色展示 warning 级环境问题', () => {
    ...
    const { container } = render(<WorktreePanel {...baseProps({
      onEnvCheck: noop,
      envHealthMap: {
        'TASK-A': { status: 'warning', issueCount: 1, result: { summary: { status: 'warning', issueCount: 1, message: 'Git 有 1 个未提交改动' } } },
      },
    })} />);

    // warningTag 存储显示“1 个环境问题”的环境标签，它应该是 warning 色而不是 error 色。
    const warningTag = [...container.querySelectorAll('.ant-tag')]
      .find((tag) => tag.textContent.includes('1 个环境问题'));
...
describe('WorktreePanel 需求流程入口', () => {
  ...
  const steps = [
    ...
    { key: 'branch-to-jira', label: '自动提取分支到 Jira', command: './jira.sh {branch}' },
  ];
  ...
  it('勾选 checkbox 步骤回调 onToggleStep(任务名, 步骤key, true)', () => {
    ...
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, onToggleStep })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
  ...
  it('已勾选的步骤复选框呈选中态', () => {
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, workflowMap: { 'TASK-A': ['start'] } })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
  ...
  it('配了执行命令的步骤渲染「执行」按钮，点击回调 onRunStepAction(task, step)', () => {
    ...
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, onRunStepAction })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
  ...
  it('步骤数超过 5 时进度用紧凑 N/M 文字展示（不撑长）', () => {
    ...
    const manySteps = Array.from({ length: 7 }).map((_, i) => ({ key: `s${i}`, label: `步骤${i}`, command: '' }));
    render(<WorktreePanel {...baseProps({ workflowSteps: manySteps, workflowMap: { 'TASK-A': ['s0', 's1'] } })} />);
  ...
  it('点击流程入口不触发面板展开/折叠（onActiveKeysChange 不被调用）', () => {
    ...
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, onActiveKeysChange })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
  ...
  it('runningSteps 命中时该步骤「执行」按钮显示 loading 并禁用', () => {
    ...
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, runningSteps: { 'TASK-A::branch-to-jira': true } })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
    ...
    const loadingBtn = [...document.querySelectorAll('.ant-btn-loading')]
      .find((btn) => btn.textContent.includes('执行中'));
  ...
  it('其他任务执行中不影响本任务按钮（按钮级 loading 隔离）', () => {
    ...
    const onRunStepAction = vi.fn();
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, onRunStepAction, runningSteps: { 'TASK-B::branch-to-jira': true } })} />);
  ...
  it('流程 Modal 提供运行全部按钮并只在有命令步骤时可用', () => {
    ...
    render(<WorktreePanel {...baseProps({ workflowSteps: steps, onRunWorkflowSteps })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
  ...
  it('失败步骤显示未通过状态、重试和查看输出', () => {
    ...
    render(<WorktreePanel {...baseProps({
      ...
      onViewLastOutput,
      lastStepOutputs: {
        'TASK-A::branch-to-jira': { status: 'error', label: '自动提取分支到 Jira', content: 'boom' },
      },
    })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
    ...
    expect(onRunStepAction).toHaveBeenCalledTimes(1);
    fireEvent.click(document.querySelector('.anticon-file-text').closest('button'));
  ...
  it('失败步骤被用户手动勾选后保持勾选态并显示已完成', () => {
    render(<WorktreePanel {...baseProps({
      ...
      lastStepOutputs: {
        'TASK-A::branch-to-jira': { status: 'error', label: '自动提取分支到 Jira', content: 'unit failed' },
      },
    })} />);
    fireEvent.click(screen.getAllByText('流程')[0]);
    ...
    const checkboxWrap = screen.getByText('自动提取分支到 Jira').closest('.ant-checkbox-wrapper');
  ...
  it('完成态命令步骤的操作按钮与步骤首行顶部对齐', () => {
    ...
    const unitTestSteps = [
      { key: 'unit-test', label: '检查单测', command: 'bash check-unit-test.sh' },
    ];
    render(<WorktreePanel {...baseProps({
...
describe('WorktreePanel 空态', () => {
  ...
  it('任务没有项目 worktree 时展开内容显示添加项目入口', () => {
    ...
    const emptyTask = { task: 'TASK-EMPTY', path: '/wt/TASK-EMPTY', worktrees: [] };
...
describe('WorktreePanel 任务状态标记', () => {
  ...
  it('已设置状态时显示对应中文状态名', () => {
    render(<WorktreePanel {...baseProps({ taskStatusMap: { 'TASK-A': 'released' } })} />);
