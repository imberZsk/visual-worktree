...
beforeAll(() => {
  console.error = (...args) => {
    ...
    if (includesConsoleText(args, 'not wrapped in act') || includesConsoleText(args, 'not wrapped in %s')) {
      throw new Error(`检测到未等待的 React 状态更新：${args.map(String).join(' ')}`);
    }
    if (includesConsoleText(args, '`imageStyle` is deprecated')) {
      throw new Error(`检测到已废弃的 antd Empty API：${args.map(String).join(' ')}`);
    }
