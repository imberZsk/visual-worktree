...
import CleanupSuggestionsModal from '../../src/ui/components/CleanupSuggestionsModal.tsx'
...
describe('CleanupSuggestionsModal', () => {
  ...
  it('删除前重新扫描，候选项已消失时不继续强制删除', async () => {
    ...
    fireEvent.click(
      document.querySelector('.ant-table-row .ant-checkbox-input')
    )
    fireEvent.click(screen.getByText('删除选中'))
    fireEvent.click(await screen.findByText('确定删除'))

    await waitFor(() =>
      expect(window.api.getSafeToRemoveWorktrees).toHaveBeenCalledTimes(2)
    )
  ...
  it('在标题旁说明清理建议的统计口径', async () => {
    ...
    renderModal()
    await waitFor(() =>
      expect(window.api.getSafeToRemoveWorktrees).toHaveBeenCalledTimes(1)
    )
