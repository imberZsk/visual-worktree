...
describe('ideaWorkflowService', () => {
  ...
  it('appendIdeaRun：插入头部', () => {
    // run1、run2 为两条测试运行记录
    const run1 = { id: 'r1', workflowId: 'w1', workflowName: 'W1', idea: '想法1', targetDir: '/tmp', startedAt: '2026-01-01', finishedAt: null, status: 'running', steps: [] };
    const run2 = { id: 'r2', workflowId: 'w1', workflowName: 'W1', idea: '想法2', targetDir: '/tmp', startedAt: '2026-01-02', finishedAt: null, status: 'running', steps: [] };
    appendIdeaRun(run1);
    appendIdeaRun(run2);
    // runs 为追加两条后的运行历史
  ...
  it('appendIdeaRun：超50条截断到50条', () => {
    // 先插入50条
    for (let i = 0; i < 50; i++) {
      appendIdeaRun({ id: `r${i}`, workflowId: 'w', workflowName: 'W', idea: `idea${i}`, targetDir: '/tmp', startedAt: '', finishedAt: null, status: 'success', steps: [] });
    }
    // 再插入第51条
    appendIdeaRun({ id: 'r_new', workflowId: 'w', workflowName: 'W', idea: 'new', targetDir: '/tmp', startedAt: '', finishedAt: null, status: 'success', steps: [] });
    // runs 为截断后的运行历史
    const runs = loadIdeaRuns();
...
describe('buildStepCommand {idea} 占位符', () => {
  it('{idea} 占位符替换正常', () => {
    // cmd 为替换后的最终命令
    const cmd = buildStepCommand('git checkout -b idea/{idea}', { idea: '用户登录优化' });
