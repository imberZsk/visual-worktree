...
describe('buildTerminalCommand', () => {
  ...
  it('Ghostty command disables inherited working directory before setting target dir', () => {
    ...
    const inheritFlagIndex = cmd.indexOf(
...
describe('resolveTerminalKind 终端类型解析与兜底链', () => {
  it('Windows 默认返回 wt→powershell→cmd 兜底链', () => {
    ...
    expect(resolveTerminalKind(undefined, () => false, 'win32')).toEqual([
      'wt',
      'powershell',
      'cmd',
    ])
  ...
  it('Windows 显式选 powershell 时提到链首，其余作兜底', () => {
    ...
    expect(resolveTerminalKind('powershell', () => false, 'win32')).toEqual([
      'powershell',
      'wt',
      'cmd',
    ])
  ...
  it('Windows 上残留的 macOS 终端名（Terminal）被忽略，走默认链', () => {
    ...
    expect(resolveTerminalKind('Terminal', () => false, 'win32')).toEqual([
      'wt',
      'powershell',
      'cmd',
    ])
  ...
  it('macOS 检测到 Ghostty 时返回 ghostty→terminal 兜底链', () => {
    ...
    expect(resolveTerminalKind(undefined, () => true, 'darwin')).toEqual([
      'ghostty',
      'terminal',
    ])
  })
  ...
  it('macOS 未装 Ghostty 时只返回 terminal（无冗余兜底）', () => {
    ...
    expect(resolveTerminalKind(undefined, () => false, 'darwin')).toEqual([
      'terminal',
    ])
  })

  it('macOS 显式选 iTerm2 时返回 iterm2→terminal 兜底链', () => {
    expect(resolveTerminalKind('iTerm2', () => false, 'darwin')).toEqual([
      'iterm2',
      'terminal',
    ])
  })
