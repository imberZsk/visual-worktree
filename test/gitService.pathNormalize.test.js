...
import {
  toPosixPath,
  getTaskNameFromWorktreeRelativePath,
...
describe('getTaskNameFromWorktreeRelativePath 跨平台切分', () => {
  ...
  it('含斜杠的多层任务名完整保留（正斜杠输入）', () => {
    ...
    expect(
      getTaskNameFromWorktreeRelativePath('alice/bugfix/PROJ-5001/projA')
    ).toBe('alice/bugfix/PROJ-5001')
  })
  ...
  it('任务名含中文与特殊字符时完整保留', () => {
    ...
    expect(
      getTaskNameFromWorktreeRelativePath(
        'alice\\bugfix\\PROJ-5001-订单批量导入页面异常\\projA'
      )
    ).toBe('alice/bugfix/PROJ-5001-订单批量导入页面异常')
  })
})
