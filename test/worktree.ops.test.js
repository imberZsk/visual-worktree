...
describe('addWorktree', () => {
  ...
  it('uses an existing branch even when that branch is already checked out by the source worktree', async () => {
    ...
    const expectedPath = (
      realpathSync.native ? realpathSync.native(target) : realpathSync(target)
    ).replace(/\\/g, '/')
    expect(
      wts.some(
        (w) =>
          w.path.replace(/\\/g, '/') === expectedPath &&
          w.branch === 'feat/in-use'
      )
    ).toBe(true)
  })
  ...
  it('创建项目 worktree 后只初始化固定说明文件', async () => {
    ...
    const res = await addWorktree(repo, target, 'feat/docs', {
      newBranch: true,
    })

    expect(res.success).toBe(true)
    expect(existsSync(join(target, 'docs'))).toBe(false)
    expect(readFileSync(join(target, 'CLAUDE.md'), 'utf8')).toContain(
      'AGENTS.md'
    )
  ...
  it('固定说明文件不阻塞干净 worktree 的安全删除', async () => {
    ...
    const target = join(
      ctx.root,
      'worktrees',
      'TASK-CLEAN-REMOVE-DOCS',
      'projCleanRemoveDocs'
    )
    await addWorktree(repo, target, 'feat/clean-remove-docs', {
      newBranch: true,
    })
  ...
  it('复用已有项目 worktree 时补齐缺失的固定说明文件', async () => {
    ...
    const res = await addWorktree(repo, target, 'feat/reuse-docs', {
      newBranch: true,
    })
  ...
  it('创建项目 worktree 时不应用自定义工作文档模板', async () => {
    ...
    // res 存储 addWorktree 创建结果。
    const res = await addWorktree(repo, target, 'feat/custom-docs', {
      newBranch: true,
  ...
  it('新建分支基于主分支 master 而非源仓库当前所在分支（如 test）', async () => {
    ...
    const res = await addWorktree(repo, target, 'feat/from-main', {
      newBranch: true,
    })
    expect(res.success).toBe(true)
  ...
  it('新建分支按配置的 mainBranches 解析起点（仓库用 main 而非 master）', async () => {
    ...
    const res = await addWorktree(repo, target, 'feat/from-main2', {
      newBranch: true,
  ...
  it('新建分支前自动 fetch 主分支，起点为远程最新主分支提交', async () => {
    ...
    const { local, seed } = makeRemoteAndClone(
      join(ctx.root, 'fetchcase'),
      'master'
    )
    ...
    const res = await addWorktree(local, target, 'feat/fetched', {
      newBranch: true,
    })
    expect(res.success).toBe(true)
  ...
  it('fetchMain:false 时不 fetch，起点回退本地已有的远程引用', async () => {
    ...
    const res = await addWorktree(local, target, 'feat/nofetch', {
      newBranch: true,
      fetchMain: false,
    })
    expect(res.success).toBe(true)
...
describe('batchAddWorktree', () => {
  ...
  it('creates worktrees for multiple projects under one task dir', async () => {
    ...
    expect(
      readFileSync(join(worktreesRoot, 'PROJ-100', 'CLAUDE.md'), 'utf8')
    ).toContain('AGENTS.md')
    expect(
      readFileSync(join(worktreesRoot, 'PROJ-100', 'AGENTS.md'), 'utf8')
    ).toContain('docs/')
    ...
    expect(existsSync(join(worktreesRoot, 'PROJ-100', 'projA', 'docs'))).toBe(
      false
    )
  })
  ...
  it('批量创建 worktree 时按自定义工作文档模板初始化任务根目录', async () => {
    ...
    expect(
      readFileSync(join(worktreesRoot, 'PROJ-101', '.ai', 'task.md'), 'utf8')
    ).toBe('task notes')
    expect(
      existsSync(join(worktreesRoot, 'PROJ-101', 'projA', '.ai', 'task.md'))
    ).toBe(false)
  })
  ...
  it('项目列表为空时仍创建任务根目录和工作文档', async () => {
    ...
    expect(
      readFileSync(join(worktreesRoot, 'PROJ-EMPTY', 'AGENTS.md'), 'utf8')
    ).toContain('docs/')
  })
  ...
  it('reports per-project failure without aborting the batch', async () => {
    ...
    writeFileSync(
      join(worktreesRoot, 'PROJ-200', 'projA', 'occupied.txt'),
      'blocking'
    )
    const results = await batchAddWorktree({
      projectPaths: [projA, projB],
...
describe('node_modules 软链接', () => {
  ...
  it('addWorktree 在 linkNodeModules:false 时不创建软链接', async () => {
    ...
    mkdirSync(join(repo, 'node_modules'), { recursive: true })
    const target = join(ctx.root, 'worktrees', 'TASK-2', 'projB')
    const res = await addWorktree(repo, target, 'feat/x', {
      newBranch: true,
      linkNodeModules: false,
    })
    expect(res.success).toBe(true)
