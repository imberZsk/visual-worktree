...
const { buildVscodeCommand, openInVscode } =
  await import('../electron/ipcHandlers.js')
...
describe('openInVscode macOS 兜底打开', () => {
  it('code 与绝对 CLI 不可用时用 open -na 透传 -n 和目标路径', async () => {
    ...
    execMock.mockImplementation((cmd, cb) => {
      ...
      const shouldSucceed = cmd.startsWith(
        'open -na "Visual Studio Code" --args -n '
      )
    ...
    expect(execMock.mock.calls[1][0]).toBe(
      'open -na "Visual Studio Code" --args -n \'/wt/TASK-A\''
    )
  })
})
