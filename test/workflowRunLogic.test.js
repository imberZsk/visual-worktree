...
} from '../src/ui/workflowRunLogic.ts'
...
describe('workflowRunLogic', () => {
  ...
  it('returns only command steps, optionally starting at a step key', () => {
    expect(getRunnableWorkflowSteps(steps).map((s) => s.key)).toEqual([
      'test',
      'build',
    ])
    expect(getRunnableWorkflowSteps(steps, 'test').map((s) => s.key)).toEqual([
      'test',
      'build',
    ])
    expect(getRunnableWorkflowSteps(steps, 'build').map((s) => s.key)).toEqual([
      'build',
    ])
    expect(
      getRunnableWorkflowSteps(steps, 'missing').map((s) => s.key)
    ).toEqual(['test', 'build'])
  })
