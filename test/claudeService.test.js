...
describe('claudeService', () => {
  describe('cwdToProjectDir', () => {
    it('将 cwd 路径转换为 projects 子目录名', () => {
      ...
      expect(cwdToProjectDir('/Users/alice/workspace')).toBe(
    ...
    it('中文路径中的非 ASCII 字符替换为连字符（与 Claude Code 的目录命名规则保持一致）', () => {
      ...
      const result = cwdToProjectDir(
        '/Users/alice/worktrees/PROJ-2001-示例任务/web-app'
      )
      // 示例任务 = 4 个中文字符，每个替换为 -
    ...
    it('遍历 ~/.claude/projects 子目录，扫描主会话 jsonl 并提取元信息', () => {
      ...
      const sess2Content = [
        ...
        JSON.stringify({
          type: 'assistant',
          cwd: '/Users/alice/work',
          timestamp: '2026-06-23T09:00:01.000Z',
          message: {
      ...
      const mockReaddirSync = (p) => {
        if (p === projectsDir)
          return ['-Users-alice-workspace', '-Users-alice-work']
        if (p === `${projectsDir}/-Users-alice-workspace`)
          return ['sess1.jsonl']
...
{"type":"assistant","message":{"model":"claude-sonnet-5","usage":{"input_tokens":200,"output_tokens":100}}}
  ...
  describe('getSessionsByTask', () => {
    ...
    function buildMockDeps({ sessions, subagentJsonl = null }) {
      ...
      for (const s of sessions) {
        const projDir =
          s.projectDir || '-Users-mock-' + s.cwd.replace(/\//g, '-')
        ...
        const rows = [
          JSON.stringify({
            type: 'user',
            cwd: s.cwd,
            timestamp: s.createdAt || '2026-06-24T10:00:00.000Z',
            message: { content: s.intent || '' },
          }),
          ...(s.userMessages || []).map((txt) =>
            JSON.stringify({ type: 'user', message: { content: txt } })
          ),
          JSON.stringify({
            type: 'assistant',
            message: {
              ...(s.messageId ? { id: s.messageId } : {}),
              model: 'claude-sonnet-5',
              usage: s.usage,
            },
          }),
      ...
      const mockExistsSync = (p) => {
        ...
        // 项目子目录
        if (Object.keys(projectDirs).some((d) => p === `${projectsDir}/${d}`))
          return true
        // jsonl 文件
        if (
          Object.values(sessContents).length > 0 &&
          Object.keys(sessContents).includes(p)
        )
          return true
        // 任务目录
        if (p.includes('/mock/worktrees/'))
          return p.endsWith('/task-A') || p.endsWith('/project1')
        // subagent 目录
      ...
      const mockReaddirSync = (p) => {
        ...
        for (const [d, files] of Object.entries(projectDirs)) {
          if (p === `${projectsDir}/${d}`) {
            ...
            if (
              subagentJsonl &&
              subagentJsonl.dir === `${projectsDir}/${d}/${sessionId}`
            ) {
          ...
          // subagent 子目录
          if (subagentJsonl && p === subagentJsonl.dir)
            return [subagentJsonl.name]
        }
        return []
      }
      const mockReadFileSync = (p) => {
        if (sessContents[p]) return sessContents[p]
        if (subagentJsonl && p === subagentJsonl.path)
          return subagentJsonl.content
    ...
    it('cwd 在任务目录下时命中', () => {
      ...
      expect(sessions[0].usage).toMatchObject({
        input: 1000,
        output: 500,
        cacheWrite: 100,
        cacheRead: 2000,
    ...
    it('用户文本中含 Jira 链接时命中（多 agent 工作流场景）', () => {
      ...
      const sessions = getSessionsByTask(
        'PROJ-1001-订单状态提醒',
        '/mock/worktrees',
        deps
      )
  ...
  describe('getTasksSummary', () => {
    it('一次扫描，多任务复用，返回各任务的汇总（无会话时为零）', () => {
      ...
      expect(summary['task-A'].usage).toEqual({
        input: 0,
        output: 0,
        cacheWrite: 0,
        cacheRead: 0,
    ...
    it('多个会话的费用与 token 全部累加（跨模型）', () => {
      ...
      const sess = [
        {
          id: 'sess-A1',
          proj: '-mock-worktrees-task-A-p1',
          cwd: '/mock/worktrees/task-A/p1',
          model: 'claude-sonnet-5',
          inp: 10000,
          out: 1000,
        },
        {
          id: 'sess-A2',
          proj: '-mock-worktrees-task-A-p2',
          cwd: '/mock/worktrees/task-A/p2',
          model: 'claude-opus-4-8',
          inp: 5000,
          out: 500,
        },
      ...
      for (const s of sess) {
        ...
        contents[p] =
          ...
          JSON.stringify({
            ...
            message: {
              model: s.model,
              usage: { input_tokens: s.inp, output_tokens: s.out },
            },
          })
      ...
      const deps = {
        ...
        existsSync: (p) =>
          p === projectsDir ||
          !!contents[p] ||
          Object.keys(projDirs).some((d) => p === `${projectsDir}/${d}`) ||
          sess.some((s) => s.cwd === p || p.startsWith(s.cwd + '/')),
        readdirSync: (p) => {
          if (p === projectsDir) return Object.keys(projDirs)
          for (const [d, f] of Object.entries(projDirs)) {
            if (p === `${projectsDir}/${d}`) return f
          }
          return []
        },
        readFileSync: (p) => {
          if (contents[p]) return contents[p]
          throw new Error('nf')
        },
        statSync: () => ({ isDirectory: () => false }),
        realpathSync: (p) => p,
