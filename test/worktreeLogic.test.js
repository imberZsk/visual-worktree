...
describe('quotePathForCopy（始终加单引号，便于粘贴到终端 cd）', () => {
  ...
  it('含空格的路径加单引号', () => {
    expect(quotePathForCopy('/Users/a/my work/proj')).toBe(
      "'/Users/a/my work/proj'"
    )
  })
...
describe('getTaskStatusMeta', () => {
  ...
  it('falls back to the default "未开始" status for unknown or empty input', () => {
    ...
    expect(getTaskStatusMeta('nope')).toMatchObject({
      key: DEFAULT_TASK_STATUS,
      label: '未开始',
    })
    expect(getTaskStatusMeta(undefined)).toMatchObject({
      key: DEFAULT_TASK_STATUS,
      label: '未开始',
    })
  })
...
describe('setTaskStatusInMap', () => {
  ...
  it('clears the status when key is empty, unknown, or the default "未开始"', () => {
    ...
    expect(
      setTaskStatusInMap({ 'TASK-1': 'released' }, 'TASK-1', undefined)
    ).toEqual({})
    expect(
      setTaskStatusInMap({ 'TASK-1': 'released' }, 'TASK-1', 'bogus')
    ).toEqual({})
    expect(
      setTaskStatusInMap(
        { 'TASK-1': 'released' },
  ...
  it('returns a copy unchanged when task name is missing', () => {
    expect(
      setTaskStatusInMap({ 'TASK-1': 'released' }, '', 'developing')
    ).toEqual({ 'TASK-1': 'released' })
  })
...
describe('loadTaskStatusMap / saveTaskStatusMap', () => {
  ...
  it('round-trips a status map through localStorage', () => {
    ...
    expect(localStorage.getItem(TASK_STATUS_STORAGE_KEY)).toBe(
      '{"TASK-1":"released"}'
    )
...
describe('任务链接多值纯逻辑', () => {
  it('normalizeTaskLinks 兼容旧版单字符串链接', () => {
    expect(normalizeTaskLinks(' https://jira.example.com/TASK-1 ')).toEqual([
  ...
  it('normalizeTaskLinkItems 按 URL 去重并保留首个名称', () => {
    ...
    ).toEqual([{ name: 'Jira', url: 'https://jira.example.com/TASK-1' }])
  })
