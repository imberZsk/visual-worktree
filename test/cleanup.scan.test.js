...
describe('getSafeToRemoveWorktrees', () => {
  ...
  it('returns a worktree whose branch is merged into main and has no uncommitted changes', async () => {
    ...
    const list = await getSafeToRemoveWorktrees(projectsRoot, worktreesRoot, [
      'master',
      'main',
    ])
    // w.path 来自 git worktree list（Windows 上为正斜杠），先归一化再按正斜杠后缀匹配，避免 join 在 Windows 产生反斜杠导致失配
    const found = list.find((w) =>
      w.path.replace(/\\/g, '/').endsWith('TASK-1/projA')
    )
  ...
  it('returns empty when projectsRoot does not exist', async () => {
    const list = await getSafeToRemoveWorktrees(
      join(ctx.root, 'nonexistent'),
      worktreesRoot,
      ['master']
    )
    expect(list).toEqual([])
