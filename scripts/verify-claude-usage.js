...
sessions.slice(0, 5).forEach((session, idx) => {
  ...
  console.log(
    `  创建时间: ${session.createdAt ? new Date(session.createdAt).toLocaleString('zh-CN') : '(无)'}`
  )
  console.log(
    `  意图: ${session.intent?.substring(0, 50) || '(无)'}${session.intent?.length > 50 ? '...' : ''}`
  )
  console.log()
})
...
console.log(
  `  → 完整路径: ${join(homedir(), '.claude', 'projects', projectDir)}`
)
console.log()

// 从当前工作区配置读取 worktree 根目录；loadConfig 已把嵌套工作区投影为运行时配置。
// 该路径由当前工作区独立保存，脚本默认复用它，避免硬编码。
...
console.log(
  '  要测试任务关联功能，请运行（第二个参数可选，省略则用上面的配置路径）：'
)
...
if (process.argv.length >= 3) {
  ...
  taskSessions.forEach((session, idx) => {
    ...
    if (session.byModel && Object.keys(session.byModel).length > 0) {
      ...
      Object.entries(session.byModel).forEach(([model, detail]) => {
        console.log(
          `    ${model}: $${detail.cost.usd.toFixed(4)} (¥${detail.cost.cny.toFixed(2)})`
        )
      })
    }
    console.log()
  ...
  console.log(
    `  总 Token: ${Object.values(totalUsage)
      .reduce((a, b) => a + b, 0)
      .toLocaleString()}`
  )
  console.log(
    `  总费用: $${totalCost.toFixed(4)} (¥${(totalCost * 7.2).toFixed(2)})`
  )
}
