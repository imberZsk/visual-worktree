  ...
  for (let i = 0; i < lines.length && i < HEAD_LINES; i++) {
    ...
    if (d.type === 'user') {
      ...
      if (txt) {
        ...
        if (!meta.intent && !txt.trimStart().startsWith('<'))
          meta.intent = txt.trim().slice(0, 200)
      }
    }
  }
...
function accumulateUsageByModel(jsonlPath, acc, deps = {}, seen = new Set()) {
  ...
  for (const line of content.split('\n')) {
    ...
    if (d.type === 'assistant' && d.message?.usage) {
      ...
      const b =
        acc[model] ||
        (acc[model] = { input: 0, output: 0, cacheWrite: 0, cacheRead: 0 })
...
function matchSessionForTask(
  ...
  try {
    ...
    const cwdReal =
      session.cwd && _existsSync(session.cwd)
        ? _realpathSync(session.cwd)
        : session.cwd
    ...
    const cwdMatch =
      cwdNorm && (cwdNorm === taskNorm || cwdNorm.startsWith(taskNorm + '/'))
    // 条件2：intent 含任务目录完整路径（同时比对原始与规范化路径，覆盖 symlink 前后写法）
    const intent = session.intent || ''
    const intentMatch =
      intent.includes(taskDir) ||
      (taskDirReal !== taskDir && intent.includes(taskDirReal))
 ...
 */
function selectSessionsForTask(
  taskName,
  worktreesRoot,
  allSessions,
  deps = {}
...
export function getTasksSummary(taskNames, worktreesRoot, deps = {}) {
  ...
  for (const taskName of taskNames) {
    ...
    const totalUsd =
      Math.round(
        sessions.reduce((s, x) => s + (x.cost?.usd || 0), 0) * 1_000_000
      ) / 1_000_000
