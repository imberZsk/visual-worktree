...
import { DEFAULT_WORK_DOCUMENT_TEMPLATES, normalizeWorkDocumentTemplates } from './taskDocsService.js';
...
const CHECKABLE_PROJECT_KINDS = new Set(['frontend', 'miniprogram', 'backend', 'backend_php', 'backend_java', 'backend_python', 'fullstack']);

// NODE_DEPENDENCY_PROJECT_KINDS 存储应按 package.json/node_modules 检查依赖的项目类型。
const NODE_DEPENDENCY_PROJECT_KINDS = new Set(['frontend', 'miniprogram', 'backend', 'fullstack']);
...
const BACKEND_SCRIPT_HINTS = [
  { label: 'node server/app/main/index', pattern: /\bnode(?:\s+--[^\s]+(?:=\S+)?)*\s+(?:\.\/)?(?:[\w.-]+\/)*(?:server|app|main|index)(?:\.[cm]?[jt]s)?\b/ },
  { label: 'ts-node server/app/main/index', pattern: /\bts-node(?:\s+--[^\s]+(?:=\S+)?)*\s+(?:\.\/)?(?:[\w.-]+\/)*(?:server|app|main|index)(?:\.[cm]?ts)?\b/ },
 ...
 */
function buildSkippedWorkDocumentEntryNames(templates = DEFAULT_WORK_DOCUMENT_TEMPLATES) {
  ...
  for (const template of normalizeWorkDocumentTemplates(sourceTemplates)) {
    ...
    const firstSegment = String(template.path || '').split('/').filter(Boolean)[0];
 ...
 */
function listProjectDirs(taskDir, workDocumentTemplates = DEFAULT_WORK_DOCUMENT_TEMPLATES) {
  // dirs 累积任务目录下的子目录（即各项目 worktree）
  const dirs = [];
  // skippedEntries 存储不应当作为项目检查的任务根级工作文档入口名。
  const skippedEntries = buildSkippedWorkDocumentEntryNames(workDocumentTemplates);
...
function scoreScriptHints(scriptsText, hints, prefix) {
  ...
  for (const hint of hints) {
    ...
    const matched = pattern ? pattern.test(normalized) : normalized.includes(label.toLowerCase());
    if (matched) {
      score += 1;
...
export function detectProjectKind(projectDir) {
  ...
  const frontendName = scoreNameHints(searchableName, FRONTEND_NAME_HINTS, '前端');
  frontendScore += frontendName.score;
  frontendReasons.push(...frontendName.reasons);
  // frontendPackage 命中的前端依赖特征
  const frontendPackage = scorePackageHints(deps, FRONTEND_PACKAGE_HINTS, '前端');
  ...
  const frontendScript = scoreScriptHints(scriptsText, FRONTEND_SCRIPT_HINTS, '前端');
  frontendScore += frontendScript.score;
  frontendReasons.push(...frontendScript.reasons);
  // backendScript 命中的后端脚本特征
  const backendScript = scoreScriptHints(scriptsText, BACKEND_SCRIPT_HINTS, '后端');
  ...
  const phpFiles = scoreFileHints(projectDir, PHP_BACKEND_FILE_HINTS, 'PHP 后端');
  phpScore += phpFiles.score;
  phpReasons.push(...phpFiles.reasons);
  // javaFiles 命中的 Java 后端文件特征
  const javaFiles = scoreFileHints(projectDir, JAVA_BACKEND_FILE_HINTS, 'Java 后端');
  javaScore += javaFiles.score;
  javaReasons.push(...javaFiles.reasons);
  // pythonFiles 命中的 Python 后端文件特征
  const pythonFiles = scoreFileHints(projectDir, PYTHON_BACKEND_FILE_HINTS, 'Python 后端');
  ...
  const confidence = kind === 'unknown' ? 0 : Math.min(1, (miniprogramScore + frontendScore + backendScore + phpScore + javaScore + pythonScore) / 6);
...
function extractPorts(text) {
  ...
  for (const m of text.matchAll(/-{1,2}p(?:ort)?[=\s]+(\d{2,5})/gi)) ports.add(Number(m[1]));
  // 匹配 PORT=3000 / PORT 3000
  for (const m of text.matchAll(/\bPORT[=\s]+(\d{2,5})/g)) ports.add(Number(m[1]));
...
function checkNodeProjectDeps(projectDir) {
  ...
  if (!existsSync(pkgPath)) return { status: 'ok', message: '非 Node 项目，跳过 Node 依赖检查', fixes: [], skipped: true };
  ...
  if (!hasNodeModules) {
    ...
    const installCmd = existsSync(join(projectDir, 'pnpm-lock.yaml'))
      ...
      : existsSync(join(projectDir, 'yarn.lock'))
        ? 'yarn install'
        : 'npm install';
    return { status: 'error', message: '未安装依赖（缺少 node_modules）', fixes: [`在 ${projectDir} 运行 ${installCmd}`] };
  }
  // 有依赖但无锁文件：依赖版本不确定，提示补锁文件
  if (!hasLock) {
    return { status: 'warning', message: '已安装依赖但缺少 lock 文件，依赖版本不确定', fixes: ['提交 lock 文件以锁定依赖版本'] };
  }
  return { status: 'ok', message: '依赖完整（node_modules + lock 齐备）', fixes: [] };
}
...
function checkPhpProjectDeps(projectDir) {
  ...
  if (!existsSync(composerPath)) {
    return { status: 'ok', message: 'PHP 项目未声明 composer.json，跳过 Composer 依赖检查', fixes: [], skipped: true };
  }
  ...
  if (!hasVendor && hasDockerRuntime) {
    return { status: 'ok', message: '检测到 Docker 运行配置，跳过本机 PHP vendor 检查', fixes: [], skipped: true };
  }
  if (!hasVendor) {
    return { status: 'error', message: '未安装 PHP 依赖（缺少 vendor）', fixes: [`在 ${projectDir} 运行 composer install`] };
  }
  if (!hasComposerLock) {
    return { status: 'warning', message: 'PHP 依赖已安装但缺少 composer.lock，依赖版本不确定', fixes: ['提交 composer.lock 以锁定依赖版本'] };
  }
  return { status: 'ok', message: 'PHP 依赖完整（vendor + composer.lock 齐备）', fixes: [] };
}
...
function checkJavaProjectDeps(projectDir) {
  ...
  const hasBuildFile = JAVA_BACKEND_FILE_HINTS.some((file) => existsSync(join(projectDir, file)));
  if (!hasBuildFile) {
    return { status: 'warning', message: '未发现 Java 构建文件（pom.xml / build.gradle）', fixes: ['确认项目根目录是否正确'] };
  }
  return { status: 'ok', message: 'Java 依赖由 Maven/Gradle 缓存管理，跳过本地依赖目录检查', fixes: [], skipped: true };
}
...
function checkPythonProjectDeps(projectDir) {
  ...
  const hasDependencyFile = ['requirements.txt', 'pyproject.toml', 'Pipfile'].some((file) => existsSync(join(projectDir, file)));
  if (!hasDependencyFile) {
    return { status: 'ok', message: 'Python 项目未发现依赖声明文件，跳过依赖检查', fixes: [], skipped: true };
  }
  // hasVirtualEnv 存储项目内常见虚拟环境目录是否存在。
  const hasVirtualEnv = ['.venv', 'venv'].some((dir) => existsSync(join(projectDir, dir)));
  if (!hasVirtualEnv) {
    return { status: 'warning', message: '未发现 Python 虚拟环境（.venv / venv）', fixes: [`在 ${projectDir} 创建虚拟环境并安装依赖`] };
  }
  return { status: 'ok', message: 'Python 依赖环境已准备（虚拟环境存在）', fixes: [] };
}
...
function checkProjectDeps(projectDir, kindInfo = null) {
  ...
  if (projectKind === 'backend_python') return checkPythonProjectDeps(projectDir);
  if (isNodeDependencyProjectKind(projectKind)) return checkNodeProjectDeps(projectDir);
  return { status: 'ok', message: '非前后端运行项目，跳过依赖检查', fixes: [], skipped: true };
}
...
function extractProjectConfigPorts(projectDir) {
  ...
  const configFiles = ['.env', '.env.local', '.env.development', 'application.properties', 'application.yml', 'application.yaml'];
  ...
  for (const file of configFiles) {
    ...
    try {
      for (const port of extractPorts(readFileSync(filePath, 'utf8'))) ports.add(port);
    } catch (e) {
      // 单个配置文件读取失败不阻断整体端口检查。
    }
  }
...
async function buildPortCheckResult(ports, emptyMessage) {
  // 未声明端口：无需检查
  if (ports.length === 0) return { status: 'ok', message: emptyMessage, fixes: [], occupied: [], skipped: true };
  ...
  return { status: 'ok', message: `端口空闲：${ports.join(', ')}`, fixes: [], occupied: [] };
}
...
async function checkProjectPorts(projectDir, kindInfo = null) {
  ...
  try {
    ...
    return { status: 'warning', message: 'package.json 解析失败', fixes: [], occupied: [] };
  }
...
function extractEnvEndpoints(envText) {
  ...
  for (const rawLine of envText.split('\n')) {
    ...
    const urlMatch = value.match(/^[a-z][a-z0-9+.-]*:\/\/(?:[^@/]*@)?([^:/?#]+):(\d{2,5})/i);
    if (urlMatch) {
    ...
    if (hostPortMatch) {
      endpoints.push({ key, host: hostPortMatch[1], port: Number(hostPortMatch[2]) });
    }
  }
  return endpoints;
}
...
async function checkProjectServices(projectDir) {
  ...
  // 无 .env：无外部服务声明，跳过
  if (!envText.trim()) return { status: 'ok', message: '无 .env，跳过服务检查', fixes: [], unreachable: [] };

  // endpoints 从 .env 解析出的待探测地址
  const endpoints = extractEnvEndpoints(envText);
  if (endpoints.length === 0) return { status: 'ok', message: '.env 中未发现 host:port 形式的服务地址', fixes: [], unreachable: [] };
  ...
  await Promise.all(
    endpoints.map(async (ep) => {
      // 探测该地址是否可建立 TCP 连接
      if (!(await probeTcp(ep.host, ep.port))) unreachable.push(`${ep.key} (${ep.host}:${ep.port})`);
    })
  );
  ...
  return { status: 'ok', message: `服务可达（${endpoints.length} 个）`, fixes: [], unreachable: [] };
}
...
async function checkProjectGit(projectDir) {
  // 非 git 仓库：跳过
  if (!existsSync(join(projectDir, '.git'))) return { status: 'ok', message: '非 git 仓库，跳过', fixes: [] };
  try {
    ...
    if (notes.length > 0) {
      // fixes 依据具体情况给出建议
      const fixes = [];
      if (!status.isClean()) fixes.push('提交或暂存改动：git add -A && git commit / git stash');
    ...
    return { status: 'warning', message: `git 状态检查失败：${e.message}`, fixes: [] };
  }
}
...
function mergeResults(items) {
  ...
  if (messages.length === 0) return { status: 'ok', message: items[0].result.message, fixes: [] };
...
async function checkDirs(dirs) {
  ...
  const [depsItems, portsItems, servicesItems, gitItems] = await Promise.all([
    Promise.all(dirs.map(async (dir) => ({ dir, result: checkProjectDeps(dir) }))),
    Promise.all(dirs.map(async (dir) => ({ dir, result: await checkProjectPorts(dir) }))),
    Promise.all(dirs.map(async (dir) => ({ dir, result: await checkProjectServices(dir) }))),
    Promise.all(dirs.map(async (dir) => ({ dir, result: await checkProjectGit(dir) }))),
  ]);
  return {
...
export async function checkEnvHealth(taskDir, roles = [], opts = {}) {
  ...
  if (projectDirs.length === 0) {
    ...
    return {
      ...
      services: empty,
      git: empty,
      summary: { status: 'ok', projectCount: 0, issueCount: 0, failedProjects: [], message: '任务目录下未找到项目' },
      projects: [],
    };
  }
  ...
  if (projectEntries.length === 0) {
    ...
    const skipped = { status: 'ok', message: '未发现需要环境检查的前后端项目', fixes: [] };
    return {
      deps: skipped,
      ports: skipped,
      services: skipped,
      git: skipped,
      summary: { status: 'ok', projectCount: 0, issueCount: 0, failedProjects: [], message: '未发现需要环境检查的前后端项目' },
      projects: [],
    };
  }

  // projects 为每个项目的类型识别与检查详情，供新 UI 显示红/绿状态和问题列表
  const projects = await Promise.all(projectEntries.map((entry) => checkProject(entry.dir, entry.kindInfo)));
  // summary 为任务级汇总状态，供 Worktree 任务行 badge 使用
  const summary = summarizeProjects(projects);
  // aggregate 为全部项目的聚合结果（不论是否配置角色都返回，供顶层卡片渲染）
  const aggregate = {
    deps: mergeResults(projects.map((project) => ({ dir: project.dir, result: project.checks.deps }))),
    ports: mergeResults(projects.map((project) => ({ dir: project.dir, result: project.checks.ports }))),
    services: mergeResults(projects.map((project) => ({ dir: project.dir, result: project.checks.services }))),
    git: mergeResults(projects.map((project) => ({ dir: project.dir, result: project.checks.git }))),
    summary,
    projects,
  };
  ...
  const validRoles = (Array.isArray(roles) ? roles : []).filter(
    (r) => r && String(r.name || '').trim() && Array.isArray(r.dirs) && r.dirs.length > 0,
  );
  ...
  for (const role of validRoles) {
    ...
    if (dirs.length === 0) {
      ...
      const miss = { status: 'warning', message: `未找到该角色的目录：${role.dirs.join(', ')}`, fixes: [] };
      byRole.push({ name: role.name, deps: miss, ports: miss, services: miss, git: miss });
      continue;
    }
    // 对该角色命中的目录执行 4 类检查
