...
function realpathSync(p) {
  ...
  return typeof _realpathSync.native === 'function'
    ? _realpathSync.native(p)
    : _realpathSync(p)
}
...
export function linkNodeModules(
  ...
  if (existsSync(target) || isSymlink(target))
    return { linked: false, reason: 'target-exists' }
 ...
 */
async function resolveMainStartPoint(
  git,
 ...
 */
async function fetchMainBranches(
  git,
  mainBranches = DEFAULT_MAIN_BRANCHES,
...
function normalizeRemoteProjectPath(pathname) {
  ...
  const projectPath = cleanPath.endsWith('.git')
    ? cleanPath.slice(0, -4)
    : cleanPath
  return projectPath
}
...
function parseWorktreePorcelain(raw, mainBranches = DEFAULT_MAIN_BRANCHES) {
  ...
  return list.map((w, idx) => ({
    ...w,
    isMain: idx === 0,
    branchIsMain: isMainBranch(w.branch, mainBranches),
  }))
}
 ...
 */
export async function scanWorktreesByTask(
  projectsRoot,
  worktreesRoot,
  opts = {}
) {
    ...
    for (const wt of wts) {
      ...
      if (!taskMap.has(taskName)) {
        taskMap.set(taskName, {
          task: taskName,
          path: join(worktreesRoot, taskName),
          worktrees: [],
        })
      }
...
export async function addWorktree(projectPath, targetPath, branch, opts = {}) {
  ...
  try {
    ...
    const startPoint = willCreateBranch
      ? await resolveMainStartPoint(git, mainBranches)
      : ''
    if (autoDetect) {
      ...
      try {
        ...
        if (isMissingBranchError(checkoutErrMessage)) {
          await git.raw(
            buildWorktreeAddArgs(branch, targetPath, true, false, startPoint)
          )
      ...
      try {
        await git.raw(
          buildWorktreeAddArgs(branch, targetPath, newBranch, false, startPoint)
        )
      } catch (addErr) {
    ...
    await ensureTaskDocsIgnored(
      git,
      projectPath,
      PROJECT_WORK_DOCUMENT_TEMPLATES
    )
    ...
    if (
      ...
      (await isExistingWorktree(projectPath, targetPath))
    ) {
      let nodeModulesLinked = false
      if (doLink) {
        nodeModulesLinked = linkNodeModules(projectPath, targetPath).linked
...
async function isExistingWorktree(projectPath, targetPath) {
  try {
    ...
    const realTarget = existsSync(targetPath)
...
export async function batchAddWorktree(params, onProgress) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // 单项失败捕获为结果，不抛出
    const res = await addWorktree(projPath, targetPath, branch, {
      newBranch,
      mainBranches,
      fetchMain,
      fetchTimeout,
 ...
 */
async function checkWorktreeSafeToRemove(
  projectPath,
  worktreePath,
  branch,
  mainBranches = DEFAULT_MAIN_BRANCHES
  ...
  try {
    ...
    const mainBranch =
      mainBranches.find((b) => branches.all.includes(b)) || mainBranches[0]
 ...
 */
export async function getSafeToRemoveWorktrees(
  projectsRoot,
  worktreesRoot,
  mainBranches = DEFAULT_MAIN_BRANCHES
) {
  ...
  for (const projDir of projectDirs) {
    try {
      ...
      for (const wt of worktrees) {
        ...
        // 检查是否可安全删除（已合并+无改动）
        const check = await checkWorktreeSafeToRemove(
          projDir,
          wt.path,
          wt.branch,
          mainBranches
