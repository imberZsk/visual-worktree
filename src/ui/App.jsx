...
import { computeActiveKeysAfterCreate, STATUS_SORT_ORDER, getTaskStatusMeta, normalizeTaskLinkItems, quotePathForCopy } from './worktreeLogic.js';
import { normalizeWorkflowSteps, DEFAULT_WORKFLOW_STEPS } from './workflowLogic.js';
import { getRunnableWorkflowSteps } from './workflowRunLogic.js';
import { filterVisibleItems, hasVisibilityKey, normalizeTaskTitleBadges, prepareVisibleItems } from './visibilityLogic.js';
import { computeHistoryPageSize, getHistoryGlobalIndex, HISTORY_PAGE_SIZE_FALLBACK } from './historyPaginationLogic.js';
import { isStepEventFor, appendStepChunk, stepRunKey } from '../core/stepOutputLog.js';
...
function getSkippedEnvWorkDocumentEntryNames(workDocumentTemplates = []) {
  ...
  const templates = Array.isArray(workDocumentTemplates) ? workDocumentTemplates : [];

  for (const template of templates) {
    // normalizedPath 存储统一分隔符后的模板相对路径文本。
    const normalizedPath = String(template?.path || '').trim().replace(/\\/g, '/');
    // firstSegment 存储模板在任务根目录下的第一层入口名。
    const firstSegment = normalizedPath.split('/').filter(Boolean)[0];
    if (firstSegment && firstSegment !== '..' && !normalizedPath.startsWith('/')) skippedNames.add(firstSegment);
  }
...
function getEnvProjectDisplayStatus(project) {
  ...
  const checks = project?.checks && typeof project.checks === 'object' ? project.checks : {};
  // envCheckStatuses 存储真正环境检查项的有效状态，排除 Git 工作区提示。
  const envCheckStatuses = ENV_ISSUE_CHECK_KEYS.map((key) => checks[key]?.status).filter(Boolean);
  ...
  if (!hasAnyCheck && (currentStatus === 'warning' || currentStatus === 'failed')) return currentStatus;
  return 'ok';
}
...
function normalizeEnvProjectForDisplay(project) {
  ...
  if (status === project?.status && issueCount === (project?.issueCount || 0)) return project;
...
function summarizeVisibleEnvProjects(projects) {
  ...
  const issueCount = projects.reduce((sum, project) => sum + (project.issueCount || 0), 0);
  // failedProjects 保留历史字段名，实际存储过滤后仍有问题的项目名。
  const failedProjects = projects.filter((project) => project.status !== 'ok').map((project) => project.name);
  ...
  const status = hasFailed ? 'failed' : (hasWarning || issueCount > 0) ? 'warning' : 'ok';
 ...
 */
function normalizeEnvHealthResultForDisplay(result, taskDir, workDocumentTemplates = []) {
  if (!Array.isArray(result?.projects)) return result;

  // skippedNames 存储需要从旧结果里过滤掉的工作文档入口名。
  const skippedNames = getSkippedEnvWorkDocumentEntryNames(workDocumentTemplates);
  // normalizedProjects 存储修正过状态的项目结果，用于兼容旧缓存。
  const normalizedProjects = result.projects.map((project) => normalizeEnvProjectForDisplay(project));
  // visibleProjects 存储过滤后的真实项目检查结果。
  const visibleProjects = normalizedProjects.filter((project) => (
    !isRootWorkDocumentEnvProject(project, taskDir, skippedNames) && !isUncheckableEnvProject(project)
  ));
  ...
  const summaryChanged = emptyTaskDirResult && (
  ...
  const emptyChecksChanged = emptyTaskDirResult && ['deps', 'ports', 'services', 'git'].some((key) => result?.[key]?.status !== 'ok');
  // changed 标记本次归一化是否实际改动了项目列表、项目状态或空目录摘要。
  const changed = visibleProjects.length !== result.projects.length
    || normalizedProjects.some((project, index) => project !== result.projects[index])
    || summaryChanged
    || emptyChecksChanged;
  ...
  if (emptyTaskDirResult) {
    ...
    const emptyCheck = { status: 'ok', message: '任务目录下未找到项目', fixes: [] };
    return { ...result, deps: emptyCheck, ports: emptyCheck, services: emptyCheck, git: emptyCheck, projects: visibleProjects, summary };
  }
 ...
 */
function normalizeEnvHealthEntryForDisplay(entry, taskDir, workDocumentTemplates = []) {
  ...
  const normalizedResult = normalizeEnvHealthResultForDisplay(entry.result, taskDir || entry.taskDir, workDocumentTemplates);
  // status 存储任务行应展示的状态。
  const status = normalizedResult ? getEnvStatusFromResult(normalizedResult) : entry.status;
  // issueCount 存储任务行问题数量，优先使用归一化后的摘要。
  const issueCount = normalizedResult?.summary?.issueCount ?? entry.issueCount ?? 0;
 ...
 */
function normalizeEnvHealthMapForDisplay(map, tasks, workDocumentTemplates = []) {
  ...
  const taskDirByName = new Map((Array.isArray(tasks) ? tasks : []).map((task) => [task.task, task.path]));
  ...
  for (const [taskName, entry] of Object.entries(sourceMap)) {
    ...
    normalizedMap[taskName] = normalizeEnvHealthEntryForDisplay(entry, taskDir, workDocumentTemplates);
  }
...
function getEnvStatusFromResult(result) {
  ...
  return status === 'ok' || status === 'warning' || status === 'failed' ? status : 'failed';
}
...
function makeEnvHealthEntry(task, result, workDocumentTemplates = []) {
  ...
  const normalizedResult = normalizeEnvHealthResultForDisplay(result, task.path, workDocumentTemplates);
...
function shouldHideEnvCheckItem(key, item) {
  ...
  if (key === 'ports' && (item?.skipped || item?.message === '未在 scripts 中发现端口声明')) return true;
  return false;
}
...
function EnvHealthResultContent({ result, token }) {
  ...
  if (result.error) return <div style={{ color: token.colorError }}>{result.error}</div>;
  ...
  if (hasProjectDetails) {
    ...
    const summary = result.summary || { status: 'failed', projectCount: result.projects.length, issueCount: 0, message: '环境检查完成' };
    // summaryColor 为顶部摘要标签颜色：warning 黄色，failed 红色。
    const summaryColor = summary.status === 'ok' ? 'success' : summary.status === 'warning' ? 'warning' : 'error';
    // allProjects 存储全部可展示项目检查结果；有问题的项目前置，避免分页时真正需要处理的卡片藏到后一页。
    const allProjects = [...result.projects].sort((a, b) => (
      (ENV_PROJECT_STATUS_SORT_ORDER[a.status] ?? 3) - (ENV_PROJECT_STATUS_SORT_ORDER[b.status] ?? 3)
    ));
    // totalPages 存储项目卡片总页数，用于修正缓存切换后的越界页码。
    const totalPages = Math.max(1, Math.ceil(allProjects.length / ENV_PROJECT_PAGE_SIZE));
    ...
    return (
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minWidth: 0 }}>
            <Tag color={summaryColor} style={{ marginInlineEnd: 0 }}>
              {summary.status === 'ok' ? '环境正常' : `${summary.issueCount || 1} 个环境问题`}
            </Tag>
        ...
        <div style={{ display: 'grid', gap: 12, minHeight: projectListMinHeight }}>
          {visibleProjects.map((project) => {
            // projectStatusColor 为单项目状态标签颜色：warning 黄色，failed 红色。
            const projectStatusColor = project.status === 'ok' ? 'success' : project.status === 'warning' ? 'warning' : 'error';
            // checks 为该项目 4 类检查结果
              ...
              <Card
                ...
                title={(
                  <Space wrap size={6}>
                    ...
                    <Tag color="blue" style={{ marginInlineEnd: 0 }}>{project.kindLabel || project.kind || '未知'}</Tag>
                    <Tag color={projectStatusColor} style={{ marginInlineEnd: 0 }}>
                      {project.status === 'ok' ? '正常' : `${project.issueCount || 1} 个问题`}
                    </Tag>
                  </Space>
                )}
              >
                <Space direction="vertical" size={8} style={{ width: '100%' }}>
                  ...
                  {['deps', 'ports', 'services', 'git'].map((key) => {
                    ...
                    const item = checks[key] || { status: 'error', message: '检查结果缺失', fixes: [] };
                    if (shouldHideEnvCheckItem(key, item)) return null;
                    return (
                      <div key={key} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                        <Tag color={getCheckTagColor(item.status)} style={{ flex: '0 0 auto', marginInlineEnd: 0 }}>
                          ...
                          {item.fixes && item.fixes.length > 0 && (
                            <ul style={{ marginTop: 4, marginBottom: 0, paddingInlineStart: 18 }}>
                              {item.fixes.map((fix, idx) => (
                                <li key={idx}><Typography.Text type="secondary">{fix}</Typography.Text></li>
                              ))}
                            </ul>
                          )}
                        </div>
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      {['deps', 'ports', 'services', 'git'].map((key) => {
        ...
        const item = result[key] || { status: 'error', message: '无结果', fixes: [] };
        if (shouldHideEnvCheckItem(key, item)) return null;
        return (
 ...
 */
function HistorySingleLineText({ text, className, strong = false, isLink = false, onClick }) {
...
export default function App() {
  // 从全局 store 取状态与动作
  const { projects, loading, filter, keyword, selectedPaths, batchProgress, config } = useStore();
  const { worktreeTasks, worktreeLoading, theme: themeMode, taskStatusMap, taskLinkMap, taskWorkflowMap, taskBlockerMap, taskEnvHealthMap, runningSteps, taskVisibility, projectVisibility } = useStore();
  const { setFilter, setKeyword, setSelectedPaths, scan, loadConfig, runBatch, scanWorktrees, toggleTheme, setTaskStatus, loadTaskStatus, setTaskLink, loadTaskLinks, toggleWorkflowStep, loadTaskWorkflow, setTaskBlocker, loadTaskBlockers, setTaskEnvHealthMap, loadTaskEnvHealth, startRunningStep, finishRunningStep, loadTaskVisibility, loadProjectVisibility, setTaskHidden, setTaskPinned, setProjectHidden, setProjectPinned } = useStore();
  ...
  const [activeView, setActiveView] = useState(() => localStorage.getItem('vw-active-view') || 'worktrees');
  ...
  const [historyPageSize, setHistoryPageSize] = useState(HISTORY_PAGE_SIZE_FALLBACK);
  ...
  const envHealthMap = useMemo(
    () => normalizeEnvHealthMapForDisplay(taskEnvHealthMap || {}, worktreeTasks, config?.workDocumentTemplates),
  ...
  useEffect(() => {
    ...
    Promise.resolve(api.loadTaskWorkflowOutput?.() ?? {})
      .then((rawOutputMap) => {
        ...
        const outputMap = rawOutputMap && typeof rawOutputMap === 'object' && !Array.isArray(rawOutputMap) ? rawOutputMap : {};
  ...
  useEffect(() => () => {
    hideTaskTimers.current.forEach((timerId) => globalThis.clearTimeout(timerId));
    hideProjectTimers.current.forEach((timerId) => globalThis.clearTimeout(timerId));
  ...
  const taskTitleBadges = useMemo(() => normalizeTaskTitleBadges(config?.taskTitleBadges), [config?.taskTitleBadges]);
  // visibleProjectsForStats 存储真正参与统计与批量判断的项目列表，隐藏项目始终排除。
  const visibleProjectsForStats = useMemo(
    () => filterVisibleItems(projects, projectVisibility, (project) => project.path, false),
    [projects, projectVisibility],
  );
  // projectBaseForDisplay 存储项目 Tab 当前展示基础列表；打开显示隐藏时隐藏项也参与搜索与查看。
  const projectBaseForDisplay = useMemo(
    () => prepareVisibleItems(projects, projectVisibility, (project) => project.path, showHiddenProjects, (a, b) => a.name.localeCompare(b.name)),
    [projects, projectVisibility, showHiddenProjects],
  );
  // 过滤后的项目列表（随筛选/搜索/数据变化重算）
  const filtered = useMemo(() => filterProjects(projectBaseForDisplay, filter, keyword), [projectBaseForDisplay, filter, keyword]);
  // 概览统计
  const stats = useMemo(() => summarize(visibleProjectsForStats), [visibleProjectsForStats]);
  // visibleSelectedPaths 存储未隐藏的已选项目路径；隐藏项目即使历史上被选中过也不计入批量操作。
  const visibleSelectedPaths = useMemo(
    () => selectedPaths.filter((path) => !hasVisibilityKey(projectVisibility, 'hidden', path)),
  ...
  const sortedTasks = useMemo(
    () => prepareVisibleItems(worktreeTasks, taskVisibility, (task) => task.task, showHiddenTasks, taskCompare),
  ...
  const visibleWorktreeTasks = useMemo(
    () => prepareVisibleItems(worktreeTasks, taskVisibility, (task) => task.task, false, taskCompare),
    [worktreeTasks, taskVisibility, taskCompare],
  );
  // hasHiddenTasks 标记是否存在已隐藏任务，用于控制 Worktree 显示隐藏入口是否可用。
  const hasHiddenTasks = Array.isArray(taskVisibility?.hidden) && taskVisibility.hidden.length > 0;
  // hasHiddenProjects 标记是否存在已隐藏项目，用于控制项目显示隐藏入口是否可用。
  const hasHiddenProjects = Array.isArray(projectVisibility?.hidden) && projectVisibility.hidden.length > 0;
  ...
  useEffect(() => {
    ...
    api.getClaudeTasksSummary(taskNames).then((summary) => {
  ...
  const claudeTotal = useMemo(() => {
    ...
    return Object.values(claudeUsageMap).reduce(
      (acc, s) => {
        // u 当前任务的 token 分项用量
        const u = s?.usage || {};
        const tokens = (u.input || 0) + (u.output || 0) + (u.cacheWrite || 0) + (u.cacheRead || 0);
        return {
    ...
    modal.confirm(withConfirmDefaults({
      ...
      onOk: async () => {
        ...
        modal.confirm(withConfirmDefaults({
          ...
          okText: '强制删除',
          onOk: async () => {
            const forced = await api.removeWorktree(wt.projectPath, wt.path, { force: true });
            if (forced.success) {
  ...
  const runEnvHealthCheck = async (task, opts = {}) => {
    ...
    setTaskEnvHealthMap({ ...useStore.getState().taskEnvHealthMap, [task.task]: checkingEntry });
    if (open) {
      ...
      setEnvCheckResult((currentResult) => (
        envCheckTaskName === task.task && envCheckTaskDir === task.path ? currentResult : null
      ));
    }
    ...
    try {
      ...
      // entry 为任务行红/绿状态缓存
      const entry = makeEnvHealthEntry(task, result, config?.workDocumentTemplates);
      setTaskEnvHealthMap({ ...useStore.getState().taskEnvHealthMap, [task.task]: entry });
  ...
  const handleEnvCheck = async (task) => {
    ...
    if (entry?.result && entry.status !== 'checking') {
      ...
      const normalizedResult = normalizeEnvHealthResultForDisplay(entry.result, task.path, config?.workDocumentTemplates);
  ...
  const handleRemoveTask = (task) => {
    ...
    modal.confirm(withConfirmDefaults({
      ...
      onOk: async () => {
        ...
        const pruneRepos = [...new Set([...prunables, ...wts].map((w) => w.projectPath))];
        ...
        modal.confirm(withConfirmDefaults({
          ...
          onOk: async () => {
            for (const wt of blocked) {
              await api.removeWorktree(wt.projectPath, wt.path, { force: true });
            }
            await Promise.all([...new Set(blocked.map((w) => w.projectPath))].map((p) => api.pruneWorktrees(p)));
            // 强制删除成功后删除任务目录并记录历史。
            const done = await finalizeRemoveTask(task, archive.docsPath);
            if (!done) return;
            message.success(`已删除任务「${task.task}」`);
          ...
          onCancel: () => {
            message.info(`已删除 ${removables.length - blocked.length} 个，保留 ${blocked.length} 个有变更的`);
            scanWorktrees();
          },
        }));
      },
  ...
  const handleCreateWorktree = async (values) => {
    ...
    const linked = results.filter((r) => r.success && r.nodeModulesLinked).length;
    // linkedTip 软链接提示后缀：有则附带说明，无则空
    const linkedTip = linked > 0 ? `，其中 ${linked} 个已软链接 node_modules` : '';
    ...
    if (fail === 0) {
      ...
      const failDetails = results
        ...
        .join('；');
      message.warning(`成功 ${ok}，失败 ${fail}（${failDetails}）${linkedTip}`, 8);
    }
    ...
    if (ok > 0) {
      ...
      if (taskDir) runEnvHealthCheck({ task: values.task, path: taskDir }, { open: false });
    }
  };
  ...
  const handleTaskHiddenChange = (taskName, hidden) => {
    ...
    setHidingTaskKeys((keys) => (keys.includes(taskName) ? keys : [...keys, taskName]));
  ...
  const handleProjectHiddenChange = (projectPath, hidden) => {
    ...
    setHidingProjectKeys((keys) => (keys.includes(projectPath) ? keys : [...keys, projectPath]));
  ...
  useEffect(() => {
    ...
    const measureHistoryPageSize = () => {
      ...
      const firstItemElement = shellElement?.querySelector('.history-task-list-item');
      // itemRectHeight 存储 getBoundingClientRect 得到的列表项高度；浏览器环境下更贴近真实渲染。
      const itemRectHeight = firstItemElement?.getBoundingClientRect?.().height || 0;
    ...
    return () => globalThis.removeEventListener?.('resize', measureHistoryPageSize);
  }, [historyOpen, taskHistory.length]);

  useEffect(() => {
    // maxHistoryPage 存储当前历史条数和 pageSize 下的最大页码，至少为 1。
    const maxHistoryPage = Math.max(1, Math.ceil(taskHistory.length / historyPageSize));
  ...
  const resolveLatestWorkflowStep = async (step) => {
    try {
      ...
      const latestSteps = normalizeWorkflowSteps(Array.isArray(latestConfig?.workflowSteps) ? latestConfig.workflowSteps : []);
  ...
  const runWorkflowStepForTask = async (task, step) => {
    ...
    if (!effectiveStep.command || !String(effectiveStep.command).trim()) {
      message.info(`「${effectiveStep.label}」未配置执行命令，请在「设置 → 流程」中填写`);
    ...
    // 关掉再点「查看实时输出」能恢复完整内容。
    runningOutputs.current[rk] = { taskName, stepKey, label: effectiveStep.label, content: '', status: 'running', code: null };
    ...
    const unsub = api.onStepOutput((evt) => {
      ...
      setStepOutput((prev) => {
        if (!prev || prev.taskName !== taskName || prev.stepKey !== stepKey) return prev;
        return { ...prev, content: slot ? slot.content : appendStepChunk(prev.content, evt.chunk) };
      });
    });
    try {
      ...
      const res = await api.runWorkflowStep({ command: effectiveStep.command, cwd: task.path, task: taskName, branch, taskName, stepKey, taskArgMode });
      ...
      const summary = [res?.stdout, res?.stderr].filter((s) => s && s.trim()).join('\n');
      const slot = runningOutputs.current[rk];
      const finalContent = slot && slot.content && slot.content.trim() ? slot.content : (summary || (slot ? slot.content : ''));
      // 更新 ref 槽为最终态
      runningOutputs.current[rk] = { taskName, stepKey, label: effectiveStep.label, content: finalContent, status: finalStatus, code: finalCode };
      // 同步到 Modal（仅当仍在展示本步骤时）
      setStepOutput((prev) => {
        if (!prev || prev.taskName !== taskName || prev.stepKey !== stepKey) return prev;
        return { ...prev, content: finalContent, status: finalStatus, code: finalCode };
      });
      if (res?.success) {
        ...
        message.error(`「${effectiveStep.label}」未通过${res?.code != null ? `（退出码 ${res.code}）` : ''}`);
      }
      return { success: !!res?.success, code: finalCode, error: res?.error };
    } catch (e) {
      // 链路异常（如 IPC 失败）兜底：在 ref 与 Modal 标记失败并追加异常信息
      const slot = runningOutputs.current[rk];
      if (slot) { slot.status = 'error'; slot.content = appendStepChunk(slot.content, `\n[执行异常] ${e.message}\n`); }
      setStepOutput((prev) => {
        if (!prev || prev.taskName !== taskName || prev.stepKey !== stepKey) return prev;
        return { ...prev, status: 'error', content: slot ? slot.content : appendStepChunk(prev.content, `\n[执行异常] ${e.message}\n`) };
      });
      ...
      if (slot) {
        ...
        const nextLastStepOutputs = { ...lastStepOutputs.current, [rk]: { ...slot } };
  ...
  const viewLoading = activeView === 'workflow'
    ? false
    : activeView === 'projects' ? loading : worktreeLoading;
  ...
  const historyPagination = taskHistory.length > historyPageSize ? {
    current: historyPage,
  ...
  const onBatchMenuClick = ({ key }) => {
    ...
    if (key === 'checkout') handleBatch('checkoutMain', {}, `切到 ${mainBranch}`);
    ...
    <Layout style={{ height: '100vh', overflow: 'hidden' }}>
        ...
        <Space>
          ...
          {(activeView === 'worktrees' || activeView === 'kanban') && (
            <Tooltip title="按任务创建 Worktree">
              <Button type="primary" icon={<PlusOutlined />} onClick={() => openCreateWorktreeModal()}>
      ...
      <Content style={{ padding: 16, overflow: 'auto' }}>
        ...
        {activeView === 'worktrees' ? (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <Space size={8}>
                <Button size="small" icon={<HistoryOutlined />} onClick={handleOpenHistory}>历史任务</Button>
                <Button size="small" icon={<DeleteOutlined />} onClick={() => setCleanupOpen(true)}>清理建议</Button>
                ...
                {claudeTotal.tokens > 0 && (
                  <Tooltip
                    title={
                      ...
                      <div style={{ minWidth: 180, lineHeight: 1.8 }}>
                        <div style={{ fontWeight: 600, marginBottom: 4 }}>所有任务累计</div>
                  ...
                  >
                    <Tag icon={<ThunderboltOutlined />} color="purple" style={{ cursor: 'help', margin: 0 }}>
                      总计 {claudeTotal.tokens >= 1000 ? `${(claudeTotal.tokens / 1000).toFixed(1)}K` : claudeTotal.tokens} · ${claudeTotal.usd.toFixed(3)}
                    </Tag>
                  </Tooltip>
                )}
              </Space>
        ...
        <Row gutter={[12, 12]} style={{ marginBottom: 12 }}>
          <Col xs={12} sm={12} md={6}><Card size="small"><Statistic title="项目总数" value={stats.total} /></Card></Col>
          <Col xs={12} sm={12} md={6}><Card size="small"><Statistic title="非主分支" value={stats.nonMain} valueStyle={{ color: '#fa8c16' }} /></Card></Col>
          <Col xs={12} sm={12} md={6}><Card size="small"><Statistic title="有未提交变更" value={stats.hasChanges} valueStyle={{ color: '#cf1322' }} /></Card></Col>
          <Col xs={12} sm={12} md={6}><Card size="small"><Statistic title="可拉取更新" value={stats.canPull} valueStyle={{ color: '#d48806' }} /></Card></Col>
        </Row>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12, alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Dropdown menu={{ items: batchMenuItems, onClick: onBatchMenuClick }} disabled={!visibleSelectedPaths.length}>
            <Button type="primary">
      ...
      {/* 批量进度弹窗 */}
      <Modal title="批量处理中" open={!!batchProgress} footer={null} closable={false}>
        {batchProgress && (
          <>
            <Progress percent={Math.round((batchProgress.done / batchProgress.total) * 100)} />
      ...
      <Modal
        title={stepOutput ? `「${stepOutput.label}」执行${stepOutput.status === 'running' ? '中…' : '输出'}` : ''}
        open={!!stepOutput}
        width={720}
        ...
        footer={
          ...
          stepOutput?.status === 'running'
            ? [<span key="hint" style={{ marginRight: 12, color: token.colorTextSecondary, fontSize: 12 }}>关闭不会中断执行，可稍后重新查看</span>, <Button key="close" onClick={() => setStepOutput(null)}>关闭</Button>]
            : [<Button key="close" onClick={() => setStepOutput(null)}>关闭</Button>]
        }
      >
        {stepOutput && (
          <>
            ...
            <div style={{ marginBottom: 8, fontSize: 12 }}>
              {stepOutput.status === 'running' && (
                <span style={{ color: token.colorPrimary }}><Spin size="small" /> 正在执行，实时输出如下…</span>
              )}
              {stepOutput.status === 'success' && (
                <span style={{ color: token.colorSuccess }}>执行成功（退出码 0）</span>
              )}
              {stepOutput.status === 'error' && (
                <span style={{ color: token.colorError }}>
                  执行失败{stepOutput.code != null ? `（退出码 ${stepOutput.code}）` : ''}
                </span>
              )}
            </div>
            ...
            <pre
              ...
              style={{ maxHeight: 420, minHeight: 120, overflow: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-all', fontSize: 12, margin: 0, background: token.colorFillQuaternary, padding: 8, borderRadius: 4 }}
            >
              {stepOutput.content || (stepOutput.status === 'running' ? '（等待输出…）' : '（无输出）')}
            </pre>
          </>
        )}
      </Modal>
      ...
      <CreateWorktreeModal
        ...
        onClose={() => { setCreateWtOpen(false); setCreateWtDefaultTask(null); }}
      />
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            onClick={() => runEnvHealthCheck({ task: envCheckTaskName, path: envCheckTaskDir }, { open: true })}
            style={{ minWidth: 92 }}
          >
            重新检查
          </Button>,
          <Button key="close" onClick={() => setEnvCheckOpen(false)}>关闭</Button>,
        ]}
        width={720}
      >
        {/* 统一 minHeight：首次检查时 loading 态有稳定高度；重新检查时保留旧结果并叠加遮罩，避免弹窗内容高度骤变（CLS）。 */}
        <div className="env-health-result-shell" style={{ minHeight: 240, position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: !envCheckResult && envCheckLoading ? 'center' : 'flex-start' }}>
          {envCheckResult ? (
        ...
        <div ref={historyListShellRef} className="history-task-list-shell">
          <List
            ...
            renderItem={(item, idx) => {
              ...
              const historyIndex = getHistoryGlobalIndex({ page: historyPage, pageSize: historyPageSize, pageIndex: idx });
              ...
              const finderTitle = hasDocsPath ? '在 Finder 显示工作记录' : '暂无工作记录归档，无法在 Finder 显示';
              // vscodeTitle 存储 VSCode 按钮的可访问标题和悬停提示；无归档目录时解释禁用原因。
              const vscodeTitle = hasDocsPath ? '用 VSCode 打开工作记录' : '暂无工作记录归档，无法用 VSCode 打开';
              // statusMeta 存储历史任务人工状态的展示元信息；空状态不渲染标签。
              const statusMeta = item.status ? getTaskStatusMeta(item.status) : null;
                      ...
                      <Tooltip title={finderTitle}>
                        <span className="history-task-action-slot">
                          <Button
                            ...
                            onClick={hasDocsPath ? () => handleOpenFinder(item.docsPath) : undefined}
                          />
                        </span>
                      </Tooltip>
                      <Tooltip title={vscodeTitle}>
                        <span className="history-task-action-slot">
                          <Button
                            ...
                            disabled={!hasDocsPath}
                            icon={<VscodeIcon />}
                            onClick={hasDocsPath ? () => handleOpenVscode(item.docsPath) : undefined}
                          />
                        </span>
                      </Tooltip>
