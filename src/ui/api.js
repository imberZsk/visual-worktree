...
import { PROJECT_VISIBILITY_STORAGE_KEY, TASK_VISIBILITY_STORAGE_KEY, loadVisibilityPrefsFromStorage, saveVisibilityPrefsToStorage } from './visibilityLogic.js';
...
const browserFallback = {
  ...
  loadConfig: async () => ({ sourceProjectsPath: '', worktreesPath: '', mainBranches: ['master', 'main'], ignoredProjects: [] }),
  ...
  selectDirectory: async () => ({ canceled: false, error: '仅桌面端可选择目录' }),
  // 浏览器降级：无 Electron 原生对话框，返回明确错误给设置页提示
  selectFile: async () => ({ canceled: false, error: '仅桌面端可选择文件' }),
  // 浏览器降级：无主进程无法跑 shell 命令，回传明确失败提示
  runWorkflowStep: async () => ({ success: false, error: '仅桌面端可执行命令' }),
  copyText: async () => true,
  removeTaskFolder: async () => ({ success: false, error: '非 Electron 环境' }),
  archiveTaskDocs: async (_taskDir, taskName) => ({ success: true, docsPath: `/tmp/visual-worktree-task-docs/${taskName || 'task'}`, archivedProjects: 0 }),
  loadTaskStatus: async () => {
    // 浏览器降级：从 localStorage 读取（与旧版兼容）
    try { const r = localStorage.getItem('vw-task-status'); return r ? JSON.parse(r) : {}; } catch { return {}; }
  },
  saveTaskStatus: async (map) => {
    try { localStorage.setItem('vw-task-status', JSON.stringify(map || {})); return true; } catch { return false; }
  },
  loadTaskLinks: async () => {
    try { const r = localStorage.getItem('vw-task-links'); return r ? JSON.parse(r) : {}; } catch { return {}; }
  },
  saveTaskLinks: async (map) => {
    try { localStorage.setItem('vw-task-links', JSON.stringify(map || {})); return true; } catch { return false; }
  },
  loadTaskVisibility: async () => loadVisibilityPrefsFromStorage(TASK_VISIBILITY_STORAGE_KEY),
  saveTaskVisibility: async (prefs) => saveVisibilityPrefsToStorage(TASK_VISIBILITY_STORAGE_KEY, prefs),
  loadProjectVisibility: async () => loadVisibilityPrefsFromStorage(PROJECT_VISIBILITY_STORAGE_KEY),
  saveProjectVisibility: async (prefs) => saveVisibilityPrefsToStorage(PROJECT_VISIBILITY_STORAGE_KEY, prefs),
  loadTaskWorkflow: async () => {
    // 浏览器降级：从 localStorage 读取任务工作流勾选映射
    try { const r = localStorage.getItem('vw-task-workflow'); return r ? JSON.parse(r) : {}; } catch { return {}; }
  },
  saveTaskWorkflow: async (map) => {
    try { localStorage.setItem('vw-task-workflow', JSON.stringify(map || {})); return true; } catch { return false; }
  },
  ...
  saveTaskWorkflowOutput: async (map) => {
    try {
      localStorage.setItem(TASK_WORKFLOW_OUTPUT_STORAGE_KEY, JSON.stringify(map || {}));
      return true;
    } catch {
      return false;
    }
  ...
  openExternalUrl: async (url) => {
    try { window.open(url, '_blank'); return { success: true }; } catch { return { success: false }; }
  },
  loadTaskHistory: async () => {
    try { const r = localStorage.getItem('vw-task-history'); return r ? JSON.parse(r) : []; } catch { return []; }
  },
  ...
  checkEnvHealth: async () => ({
    ...
    summary: { status: 'ok', projectCount: 0, issueCount: 0, failedProjects: [], message: '非 Electron 环境' },
    projects: [],
  }),
  loadTaskEnvHealth: async () => {
    try { const r = localStorage.getItem('vw-task-env-health'); return r ? JSON.parse(r) : {}; } catch { return {}; }
  },
  saveTaskEnvHealth: async (map) => {
    try { localStorage.setItem('vw-task-env-health', JSON.stringify(map || {})); return true; } catch { return false; }
  },
  loadTaskBlockers: async () => {
    try { const r = localStorage.getItem('vw-task-blockers'); return r ? JSON.parse(r) : {}; } catch { return {}; }
  },
  saveTaskBlockers: async (map) => {
    try { localStorage.setItem('vw-task-blockers', JSON.stringify(map || {})); return true; } catch { return false; }
  },
  loadIdeaWorkflows: async () => {
    try { const r = localStorage.getItem('vw-idea-workflows'); return r ? JSON.parse(r) : []; } catch { return []; }
  },
  saveIdeaWorkflows: async (defs) => {
    try { localStorage.setItem('vw-idea-workflows', JSON.stringify(defs || [])); return true; } catch { return false; }
  },
  loadIdeaRuns: async () => {
    try { const r = localStorage.getItem('vw-idea-runs'); return r ? JSON.parse(r) : []; } catch { return []; }
  },
