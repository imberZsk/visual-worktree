...
// 如「开始 / 审查需求方案 / 单测 / 上传方案到飞书 / 提取分支到 Jira / Jira 评论 / 测试用例 / 查看 Jira 预算时间」。
...
export function normalizeWorkflowSteps(steps) {
  ...
  if (!Array.isArray(steps)) return DEFAULT_WORKFLOW_STEPS.map((s) => ({ ...s }));
  ...
  for (const raw of steps) {
    ...
    let base = String(raw?.key ?? '').trim() || slugifyLabel(label);
    ...
    result.push({ key, label, command, autoCheckOnSuccess, stopOnFailure, taskArgMode });
  }
  return result;
}
