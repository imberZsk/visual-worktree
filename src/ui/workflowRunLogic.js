import { isStepDone } from './workflowLogic.js';
 ...
 */
export function getWorkflowStepRunStatus(step, taskName, workflowMap = {}, runningSteps = {}, lastStepOutputs = {}) {
 ...
 */
export function getWorkflowTaskRunSummary(steps = [], taskName, workflowMap = {}, runningSteps = {}, lastStepOutputs = {}) {
  // summary 存储逐步扫描后得到的任务级状态聚合。
  const summary = { hasRunning: false, hasFailed: false, failedCount: 0, runningCount: 0, lastFailedStepLabel: '' };
  for (const step of Array.isArray(steps) ? steps : []) {
    // status 存储当前步骤的展示状态。
    const status = getWorkflowStepRunStatus(step, taskName, workflowMap, runningSteps, lastStepOutputs);
    if (status === 'running') {
    ...
    if (status === 'failed') {
      ...
      summary.lastFailedStepLabel = step?.label || lastStepOutputs?.[stepRunKey(taskName, step?.key)]?.label || '';
    }
  }
  return summary;
}
...
export function getRunnableWorkflowSteps(steps = [], startKey = '') {
  ...
  const foundIndex = startKey ? list.findIndex((step) => step?.key === startKey) : -1;
