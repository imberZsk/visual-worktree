...
import { Collapse, Tag, Button, Space, Empty, Tooltip, Dropdown, Popover, Modal, Checkbox, theme, Spin } from 'antd';
import { EyeInvisibleOutlined, EyeOutlined, FolderOpenOutlined, DeleteOutlined, WarningOutlined, CopyOutlined, DownOutlined, PlusOutlined, LinkOutlined, RocketOutlined, UnorderedListOutlined, ThunderboltOutlined, CheckCircleOutlined, FileTextOutlined, PushpinFilled, PushpinOutlined, GitlabOutlined, ConsoleSqlOutlined } from '@ant-design/icons';
import { TASK_STATUSES, getTaskStatusMeta, normalizeTaskLinkItems } from '../worktreeLogic.js';
...
function TaskStatusControl({ taskName, statusKey, onChange }) {
  ...
  const menuItems = TASK_STATUSES.map((s) => ({
    key: s.key,
    label: <Tag color={s.color} style={{ marginInlineEnd: 0 }}>{s.label}</Tag>,
  }));
  return (
...
function EnvHealthStatusTag({ task, entry, onClick }) {
  ...
  const issueCount = entry?.issueCount || entry?.result?.summary?.issueCount || 0;
  ...
  const color = status === 'ok' ? 'success' : status === 'warning' ? 'warning' : status === 'failed' ? 'error' : status === 'checking' ? 'processing' : 'default';
  ...
  const tooltipText = status === 'warning' || status === 'failed'
    ...
    return (
      <Tooltip title={`打开 GitLab：${entry.label}`}>
        <Button
          ...
          icon={<GitlabOutlined />}
          onClick={(e) => { e.stopPropagation(); onOpenUrl?.(entry.url); }}
        />
      </Tooltip>
    );
  }
  // menuItems 存储多项目任务的 GitLab 下拉菜单项，每项打开对应项目仓库。
  const menuItems = validEntries.map((entry) => ({ key: entry.key, label: entry.label }));
 ...
 */
function WorkflowControl({ taskName, task, steps = [], workflowMap = {}, onToggleStep, onRunStepAction, onRunWorkflowSteps, runningSteps = {}, lastStepOutputs = {}, onViewLastOutput, onViewCurrentOutput }) {
  ...
  const taskRunSummary = getWorkflowTaskRunSummary(steps, taskName, workflowMap, runningSteps, lastStepOutputs);
    ...
    <div style={{ maxHeight: '60vh', overflowY: 'auto', paddingInlineEnd: 4 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 12, color: token.colorTextSecondary }}>已完成 {progress.done}/{progress.total} 步</div>
          {taskRunSummary.hasFailed && (
            <div style={{ marginTop: 2, fontSize: 12, color: token.colorError }}>
      ...
      {steps.map((step) => {
        ...
        const runStatus = getWorkflowStepRunStatus(step, taskName, workflowMap, runningSteps, lastStepOutputs);
        // failed 标记该步骤最近一次执行失败，决定按钮文案和状态标签。
        const failed = runStatus === 'failed';
        // done 为最终展示勾选态；执行失败会在上层撤销 rawDone，之后用户手动勾选应被如实展示。
        const done = rawDone;
        // statusTag 当前步骤的状态标签；idle 不渲染，降低视觉噪音。
        const statusTag = runStatus === 'running'
          ? <Tag color="processing" style={{ marginInlineEnd: 0, flexShrink: 0 }}>执行中</Tag>
          : runStatus === 'failed'
            ? <Tag color="error" style={{ marginInlineEnd: 0, flexShrink: 0 }}>未通过</Tag>
            : runStatus === 'success'
              ? <Tag color="success" style={{ marginInlineEnd: 0, flexShrink: 0 }}>已完成</Tag>
              : null;
        return (
          <div
            key={step.key}
            data-testid={`workflow-step-row-${step.key}`}
            style={{ padding: '8px 0', borderBottom: `1px solid ${token.colorBorderSecondary}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}
          >
            {/* 文案前的勾选框：标记该步是否完成 */}
            <div style={{ flex: '1 1 260px', minWidth: 0 }}>
              ...
              >
                <Checkbox
                  checked={done}
                  onChange={(e) => onToggleStep?.(taskName, step.key, e.target.checked)}
                  style={{ maxWidth: '100%', minWidth: 0, flex: '0 1 auto' }}
                >
                  <SingleLineText text={step.label} inline style={{ maxWidth: 300 }} />
                </Checkbox>
                {statusTag}
              </div>
            </div>
            ...
            <div
              ...
              style={{ flexShrink: 0, alignSelf: 'flex-start', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end', gap: 4, maxWidth: 260 }}
            >
              {(running || hasLastOutput) && (
                <Tooltip title={running ? '查看实时输出' : '查看上次输出'}>
                  <Button
                    ...
                    onClick={() => (running ? onViewCurrentOutput : onViewLastOutput)?.(task, step)}
                  />
                </Tooltip>
              )}
  ...
  return (
    ...
    <span onClick={(e) => e.stopPropagation()} style={{ display: 'inline-flex' }}>
      <Tooltip title={progress.total > 0 ? `需求流程：已完成 ${progress.done}/${progress.total} 步` : '查看/操作需求流程'}>
        ...
        >
          ...
          {!taskRunSummary.hasRunning && taskRunSummary.hasFailed && (
            <WarningOutlined style={{ color: token.colorError, marginInlineStart: 5 }} />
          )}
          ...
          {progress.total > 0 && (
            <span style={{ marginInlineStart: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'flex-start', minWidth: 46, verticalAlign: 'middle' }}>
              {allDone && (
                  ...
                  {Array.from({ length: progress.total }).map((_, i) => (
                    <span
                      ...
                      style={{
                        ...
                        background: i < progress.done ? token.colorPrimary : token.colorFillSecondary,
                      }}
                    />
                  ))}
                  {/* 进度文字补充：圆点旁附 N/M 文字，未开始时淡化以降低存在感（不制造「待办焦虑」） */}
                  <span style={{ marginInlineStart: 2, fontSize: 11, color: notStarted ? token.colorTextQuaternary : token.colorTextSecondary }}>
              ...
              {!allDone && progress.total > PROGRESS_DOT_MAX && (
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 12, color: notStarted ? token.colorTextQuaternary : token.colorTextSecondary }}>
                  <CheckCircleOutlined style={{ fontSize: 11, color: progress.done > 0 ? token.colorPrimary : token.colorTextQuaternary }} />
                  {progress.done}/{progress.total}
                </span>
              )}
            </span>
...
function wtStatusTags(wt) {
  ...
  // 落后远程
  if (wt.behind > 0) tags.push(<Tag color="gold" key="behind">落后 {wt.behind}</Tag>);
  return tags;
}
 ...
 * @param {boolean} [props.showHiddenTasks] - 是否正在展示隐藏任务
 ...
 */
export default function WorktreePanel({ tasks, loading, activeKeys, onActiveKeysChange, onOpenFinder, onOpenVscode, onOpenTerminal, onCopyPath, onRemove, onRemoveTask, onPrune, taskStatusMap = {}, onTaskStatusChange, taskLinkMap = {}, onTaskLinkChange, onOpenUrl, onAddWorktree, onEnvCheck, envHealthMap = {}, cicdLinks = {}, claudeUsageMap = {}, workflowSteps = [], workflowMap = {}, onToggleStep, onRunStepAction, onRunWorkflowSteps, runningSteps = {}, lastStepOutputs = {}, onViewLastOutput, onViewCurrentOutput, hiddenTaskKeys = [], pinnedTaskKeys = [], hidingTaskKeys = [], showHiddenTasks = false, onTaskHiddenChange, onTaskPinnedChange, taskTitleBadges }) {
   ...
   */
  const isTaskHidden = (taskName) => hasVisibilityKey(taskVisibility, 'hidden', taskName);
   ...
   */
  const isTaskPinned = (taskName) => hasVisibilityKey(taskVisibility, 'pinned', taskName);
  ...
  if (!tasks || tasks.length === 0) {
    return (
      <div style={{ minHeight: 240 }}>
        {loading ? null : (
          <Empty description="暂无 worktree。点击右上角「创建 Worktree」按任务批量创建" style={{ marginTop: 60 }} />
        )}
      </div>
    );
  }
  ...
  const items = (tasks || []).map((t) => {
    ...
    // primaryTaskUrl 存储该任务的第一条链接 URL，用于单链接时快捷打开
    const primaryTaskUrl = taskLinks[0]?.url || '';
    ...
    const linkPopoverContent = (
      <Space direction="vertical" size={4}>
        ...
        />
        <Space size={4}>
          <Button size="small" onClick={() => handleSaveLink(t.task)}>保存</Button>
          {hasTaskLinks && (
            <Button
              size="small"
              danger
              onClick={() => { onTaskLinkChange?.(t.task, []); setLinkPopoverTask(null); }}
            >
              清除
            </Button>
          )}
    ...
    const openTaskLinkPopover = () => {
      setLinkInputVal(taskLinks.length > 0 ? taskLinks : [{ name: '', url: '' }]);
      ...
      label: (
        <Space wrap>
          ...
          {titleBadges.claudeUsage && (
            <span onClick={(e) => e.stopPropagation()} style={{ display: 'inline-flex' }}>
              <ClaudeUsageTag taskName={t.task} summary={claudeUsageMap[t.task]} />
            </span>
          )}
          {hasPrunable && (
        ...
        <Space size={0}>
          ...
          <Tooltip title={taskPinned ? '取消置顶任务' : '置顶任务'}>
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); onTaskPinnedChange?.(t.task, !taskPinned); }}
            />
          </Tooltip>
          ...
          <Tooltip title={taskHidden ? '恢复显示任务' : '隐藏任务'}>
            <Button
              ...
              disabled={taskHiding}
              onClick={(e) => { e.stopPropagation(); onTaskHiddenChange?.(t.task, !taskHidden); }}
            />
          </Tooltip>
          {/* 为此任务追加创建 worktree */}
          <Tooltip title="为此任务添加项目 Worktree">
            <Button
              ...
              type="link"
              icon={<PlusOutlined />}
              onClick={(e) => { e.stopPropagation(); onAddWorktree?.(t); }}
            />
          </Tooltip>
          ...
          <span onClick={(e) => e.stopPropagation()} style={{ display: 'inline-flex' }}>
            <Popover
              ...
              onOpenChange={(v) => { if (v) openTaskLinkPopover(); else setLinkPopoverTask(null); }}
              content={linkPopoverContent}
              title="绑定需求链接"
              trigger="click"
              placement="bottomRight"
            >
              <Tooltip title={hasTaskLinks ? '已绑定链接，点击管理' : '绑定 Jira/飞书需求/工单链接'}>
                <Button
                  size="small"
                  type="link"
                  icon={<LinkOutlined style={{ color: hasTaskLinks ? token.colorPrimary : undefined }} />}
                  onClick={(e) => { e.stopPropagation(); openTaskLinkPopover(); }}
                />
              </Tooltip>
            </Popover>
          </span>
          ...
          <Tooltip title="在 VSCode 中打开">
            <Button
              ...
              type="link"
              icon={<VscodeIcon />}
              onClick={(e) => { e.stopPropagation(); onOpenVscode(t.path); }}
            />
          </Tooltip>
          ...
          <Tooltip title="在 Finder 中打开">
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); onOpenFinder(t.path); }}
            />
          </Tooltip>
          ...
          <Tooltip title="复制路径">
            <Button
              ...
              type="link"
              icon={<CopyOutlined />}
              onClick={(e) => { e.stopPropagation(); onCopyPath(t.path); }}
            />
          </Tooltip>
          ...
          <Tooltip title="在终端中打开">
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); onOpenTerminal(t.path); }}
            />
          </Tooltip>
      ...
      children: (
        <div>
          ...
          {t.worktrees.length === 0 ? (
            ...
            <div
              key={wt.path}
              style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${token.colorBorderSecondary}` }}
            >
              <Space direction="vertical" size={0} style={{ minWidth: 0, flex: 1 }}>
                <Space wrap size={4}>
                  <Tag color="geekblue" style={{ maxWidth: 220, marginInlineEnd: 0 }}>
                    <SingleLineText text={wt.project} inline style={{ maxWidth: 200 }} />
                  </Tag>
                  <SingleLineText text={wt.branch || '(detached)'} as="code" inline style={{ maxWidth: 360, fontSize: 12 }} />
                  {wtStatusTags(wt)}
                </Space>
                <SingleLineText text={wt.path} style={{ color: token.colorTextSecondary, fontSize: 12 }} tooltipPlacement="bottom" />
              </Space>
              <Space size={4}>
                {/* 失效 worktree 提供清理按钮 */}
                {(wt.prunable || wt.missing) ? (
                  <Button size="small" onClick={() => onPrune(wt)}>清理</Button>
                ) : (
                  <>
                    {/* CI/CD 流水线：当该项目在配置中有对应 URL 时才显示 */}
                    {cicdLinks[wt.project] && (
                      <Tooltip title="打开 CI/CD 流水线">
                        <Button size="small" icon={<RocketOutlined />} onClick={() => onOpenUrl?.(cicdLinks[wt.project])} />
                      </Tooltip>
                    )}
                    <Tooltip title="在 VSCode 中打开">
                      <Button size="small" icon={<VscodeIcon />} onClick={() => onOpenVscode(wt.path)} />
                    </Tooltip>
                    ...
                    <Tooltip title="在 Finder 中打开">
                      <Button size="small" icon={<FolderOpenOutlined />} onClick={() => onOpenFinder(wt.path)} />
                    </Tooltip>
                    <Tooltip title="复制路径">
                      <Button size="small" icon={<CopyOutlined />} onClick={() => onCopyPath(wt.path)} />
                    </Tooltip>
                    <Tooltip title="在终端中打开">
                      <Button size="small" icon={<ConsoleSqlOutlined />} onClick={() => onOpenTerminal(wt.path)} />
                    </Tooltip>
                    <Tooltip title="删除此 worktree">
                      <Button size="small" danger icon={<DeleteOutlined />} onClick={() => onRemove(wt)} />
                    </Tooltip>
                  </>
                )}
              </Space>
