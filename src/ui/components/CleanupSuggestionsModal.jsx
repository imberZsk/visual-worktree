import { Modal, Table, Button, Checkbox, Space, Typography, message } from 'antd';
...
export default function CleanupSuggestionsModal({ open, onClose, onDeleted }) {
    ...
    Modal.confirm(withConfirmDefaults({
      ...
      onOk: async () => {
        ...
        try {
          ...
          const latestMap = new Map(latestList.map((item) => [item.path, item]));
          // itemsToDelete 存储仍然在最新安全候选中的选中项。
          const itemsToDelete = selectedPaths.map((path) => latestMap.get(path)).filter(Boolean);
          ...
          if (itemsToDelete.length === 0) {
            ...
            for (const item of itemsToDelete) {
              try {
                ...
                const result = await window.api.removeWorktree(item.projectPath, item.path, {});
  ...
  // totalSize 选中项的总大小
  const totalSize = list.reduce((sum, item) => (selected.has(item.path) ? sum + item.sizeBytes : sum), 0);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 210,
      ellipsis: { showTitle: false },
      render: (branch) => <SingleLineText text={branch || '(detached)'} as="code" />,
    },
    { title: '大小', dataIndex: 'sizeBytes', width: 100, render: formatSize },
    { title: '最后修改', dataIndex: 'lastModified', width: 160, render: formatTime },
  ];
    ...
    <Modal
      title={(
        <Space size={8} align="baseline" wrap>
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            当前扫描结果：仅统计已合并到主分支且无未提交改动的 worktree，删除前会重新检查。
          </Text>
        </Space>
      )}
      open={open}
      ...
      footer={
        <Space>
          ...
          <Button type="primary" danger icon={<DeleteOutlined />} onClick={handleDelete} loading={deleting} disabled={selected.size === 0}>
            删除选中
          </Button>
        </Space>
      }
