...
function toEditableLinks(value) {
  ...
  const directLinks = Array.isArray(value) ? value.map(toEditableLinkItem) : null;
  if (directLinks) return directLinks.length > 0 ? directLinks : [{ name: '', url: '' }];
...
export default function TaskLinksEditor({ value, onChange, width = '100%' }) {
  ...
  const handleRemove = (index) => {
    ...
    const nextLinks = editableLinks.length > 1
  ...
  const handlePaste = (index) => (event) => {
    ...
    const pastedLinks = normalizeTaskLinkItems(event.clipboardData?.getData('text') || '');
  ...
  return (
    <Space direction="vertical" size={6} style={{ width }}>
