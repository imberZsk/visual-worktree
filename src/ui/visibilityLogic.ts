...
export const DEFAULT_TASK_TITLE_BADGES = Object.fromEntries(
  TASK_TITLE_BADGE_ITEMS.map((item) => [item.key, true])
)
...
export function setVisibilityKey(prefs, listKey, key, enabled) {
  ...
  const normalizedKey = normalizeVisibilityKey(key)
  if (!normalizedKey || !VISIBILITY_LIST_KEYS.includes(listKey))
...
export function filterVisibleItems(items, prefs, getKey, showHidden = false) {
  ...
  return sourceItems.filter(
    (item) => !hiddenKeys.has(normalizeVisibilityKey(getKey(item)))
  )
}
 ...
 */
export function prepareVisibleItems(
  items,
  prefs,
  getKey,
  showHidden = false,
