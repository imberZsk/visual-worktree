...
async function loadClaudeSettingsEnv() {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(env)) {
      ...
      if (
        typeof v === 'string' &&
        STEP_ENV_PREFIXES.some((p) => k.startsWith(p))
      ) {
        result[k] = v
      }
    }
...
export function buildVscodeCommand(
  ...
  const quoted =
    platform === 'win32' ? winQuote(targetPath) : shellSingleQuote(targetPath)
  ...
  const needNewWindow =
    /(^|\/|\\)code(\s|$)/.test(tpl) &&
    !/(^|\s)(-n|--new-window|-r|--reuse-window)(\s|$)/.test(tpl)
  // injected 为补齐新窗口参数后的模板
  const injected = needNewWindow
    ? tpl.replace(/(^|\/|\\)code(\s|$)/, (m) => `${m.trimEnd()} -n `)
    : tpl
  // 模板含 {path} 占位符则替换，否则在末尾拼接路径，兼容两种写法
  return injected.includes('{path}')
    ? injected.replace('{path}', quoted)
    : `${injected} ${quoted}`
}
  ...
  return new Promise((resolve) => {
    ...
    exec(buildVscodeCommand(template, targetPath, platform), (err) => {
      ...
      if (cliPath) {
        ...
        const quotedCliPath = isWin
          ? winQuote(cliPath)
          : shellSingleQuote(cliPath)
    ...
    function fallbackOpen() {
      ...
      const fallbackCommands = isWin
        ? [`start "" code -n ${quoted}`]
        : [
            `open -na "Visual Studio Code" --args -n ${quoted}`,
            `open -a "Visual Studio Code" ${quoted}`,
          ]
      /**
...
function hasHighConfidenceErrorOutput(output) {
  ...
  return /^\s*(?:\[错误\]|\[失败\]|\[未通过\]|\[ERROR\]|\[Error\]|ERROR:|Error:|错误[:：]|失败[:：]|未通过[:：])/m.test(
    text
  )
}
...
export function runWorkflowStep(
  ...
  const finalCmd = buildStepCommand(
    command,
    { path: cwd, task, branch },
    { taskArgMode }
  )
  return new Promise((resolve) => {
    ...
    const child = spawn(shell.cmd, [...shell.args, finalCmd], {
      cwd,
      env: childEnv,
    })
...
export function registerIpcHandlers(ipcMain, deps = {}) {
  ...
  ipcMain.handle(IPC.GET_PROJECT_STATUS, async (_e, projectPath, opts = {}) => {
    const cfg = loadConfig(configBaseDir)
    return gitService.getProjectStatus(projectPath, {
      mainBranches: cfg.mainBranches,
      ...opts,
    })
  })
  ...
  ipcMain.handle(
    IPC.ADD_WORKTREE,
    async (_e, projectPath, targetPath, branch, opts) => {
  ...
  ipcMain.handle(
    IPC.REMOVE_WORKTREE,
    async (_e, projectPath, worktreePath, opts) => {
  ...
  ipcMain.handle(IPC.BATCH_ADD_WORKTREE, async (_e, params) => {
    ...
    return gitService.batchAddWorktree(
      ...
      {
        worktreesRoot: cfg.worktreesPath,
        mainBranches: cfg.mainBranches,
  ...
  // 保存配置
  ipcMain.handle(IPC.SAVE_CONFIG, async (_e, config) =>
    saveConfig(config, configBaseDir)
  )
  ...
  ipcMain.handle(IPC.ARCHIVE_TASK_DOCS, (_e, taskDir, taskName) => {
    ...
    const cfg = loadConfig(configBaseDir)
    return archiveTaskDocs(
      taskDir,
      taskName,
      TASK_DOCS_ARCHIVE_ROOT,
      cfg.workDocumentTemplates
    )
  })
  ...
  // 保存任务链接映射
  ipcMain.handle(IPC.SAVE_TASK_LINKS, (_e, map) =>
    writeJsonFile(LINKS_FILE, map || {})
  )

  // 读取任务隐藏/置顶偏好
  ipcMain.handle(IPC.LOAD_TASK_VISIBILITY, () =>
    readJsonFile(TASK_VISIBILITY_FILE)
  )

  // 保存任务隐藏/置顶偏好
  ipcMain.handle(IPC.SAVE_TASK_VISIBILITY, (_e, prefs) =>
    writeJsonFile(TASK_VISIBILITY_FILE, prefs || {})
  )

  // 读取项目隐藏/置顶偏好
  ipcMain.handle(IPC.LOAD_PROJECT_VISIBILITY, () =>
    readJsonFile(PROJECT_VISIBILITY_FILE)
  )

  // 保存项目隐藏/置顶偏好
  ipcMain.handle(IPC.SAVE_PROJECT_VISIBILITY, (_e, prefs) =>
    writeJsonFile(PROJECT_VISIBILITY_FILE, prefs || {})
  )
  ...
  ipcMain.handle(IPC.SAVE_TASK_WORKFLOW, (_e, map) =>
    writeJsonFile(WORKFLOW_FILE, map || {})
  )

  // 读取任务工作流步骤最近一次执行输出缓存（不存在/损坏时回退空对象）
  ipcMain.handle(IPC.LOAD_TASK_WORKFLOW_OUTPUT, () =>
    readJsonFile(WORKFLOW_OUTPUT_FILE)
  )

  // 保存任务工作流步骤最近一次执行输出缓存（目录不存在时自动创建）
  ipcMain.handle(IPC.SAVE_TASK_WORKFLOW_OUTPUT, (_e, map) =>
    writeJsonFile(WORKFLOW_OUTPUT_FILE, map || {})
  )
  ...
  ipcMain.handle(IPC.GET_SAFE_TO_REMOVE_WORKTREES, async () => {
    ...
    return gitService.getSafeToRemoveWorktrees(
      cfg.sourceProjectsPath,
      cfg.worktreesPath,
      cfg.mainBranches
    )
  })
