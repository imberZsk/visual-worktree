...
- **批量操作**：批量拉取更新、切回各仓库实际使用的主分支，并显示逐项进度。
- **任务化 worktree**：按 `worktreesRoot/{任务名}/{项目名}` 创建和聚合 worktree，适合一个需求跨多个仓库的场景。
- **任务工作流**：为每个任务记录流程步骤、任务状态、备注、外部链接和执行输出，并支持按工作区新增、删除、排序和配置任务状态的看板归类。
- **独立工作区**：每套源项目与 Worktree 路径分别保存流程、工具、展示和其他系统设置，切换时互不影响。
...
- 直接下载 DMG/EXE 安装包的普通用户不需要安装 Node.js 或 pnpm
- Windows 额外建议：
...
```bash
pnpm install
pnpm run verify:electron
pnpm dev
```

Electron 43 通过项目 `postinstall` 在 `pnpm install` 阶段调用官方 `install-electron` 命令，并显式传入公开的 npmmirror 镜像；`.npmrc` 同时保留相同镜像配置，兼容会读取该配置的安装工具。显式环境变量不能省略，因为 pnpm 11 不会把未知的 `.npmrc` 键稳定导出给生命周期脚本。校验命令会检查 `node_modules/electron/path.txt`、`dist/` 以及平台可执行文件。`pnpm dev` 也会先执行同一校验，缺失运行时时会直接提示重新安装，而不会在启动阶段静默等待大文件下载。依赖图未变化但本地运行时被删除时，可执行 `pnpm run postinstall` 定向恢复。
...
新安装首次启动时，应用会显示专用初始化弹层，直接配置：

- **源项目根目录**：存放多个 Git 仓库的目录
- **Worktree 根目录**：按任务生成 worktree 的目录

保存后即可进入应用；主分支、忽略列表、工具和流程等高级选项可稍后在设置中调整。
